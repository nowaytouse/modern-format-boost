...
### 39. 代码风格：match 中的 `_`（下划线）

项目中大量出现的 `_ => Ok(false)`、`_ => unreachable!()` 等是 **Rust 惯用写法**，不是错误或占位符：

- **`_` 在 match 中**：表示「通配符」分支，匹配所有未被前面分支覆盖的情况，且不绑定到变量。
- **为何用 `_`**：当默认分支不需要使用被匹配的值时，用 `_` 明确表示「不关心具体值」，既符合习惯，也避免未使用变量警告（若写 `other =>` 且不用 `other` 会触发 warning）。
- **无需修改**：这是语言标准用法，无需替换为具名变量。
