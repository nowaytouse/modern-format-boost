...
1. Backfill `directory_meme_score` and `directory_meme_hint`:
   - Command:
     ...
     ./.venv_training/bin/python scripts/backfill_directory_scores.py
...
2. Retrain model:
   - Command:
     ./.venv_training/bin/python scripts/training_pipeline.py train --connstr "host=localhost dbname=modern_format_boost"
...
- `scripts/backfill_directory_scores.py` (added migration to create columns)
- `scripts/training_pipeline.py` (no changes; used for training)
