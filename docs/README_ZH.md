...
直接像素→JXL 不再把渐进与合成噪声留给编码器自动决定：明确传入 `--progressive_dc=0`、`--responsive=0`、`--noise=0`、`--photon_noise_iso=0`，且不请求 `-p`、渐进 AC 或量化渐进 AC。这些开关关闭的是人工生成噪声，不是破坏性的预先降噪；无损 `d=0` 仍逐像素保留真实传感器噪声，有损路径可能平滑纹理，但不会用合成噪声冒充原始纹理。不可变的 JPEG 比特流转码不接收这些像素编码参数，原 JPEG 与 JBRD 重建证明仍是权威。

体积影响已通过实测确认，而不是写成通用百分比。本机 `cjxl` v0.13 对两张确定性 512×512、`d=1/e7` 样本开启 `-p` 后，体积分别增加 20.4% 与 2.83%，峰值内存约增加 23% 与 32%；一张纹理 `d=3` 样本开启自适应人工噪声后从 19,052 字节变为 18,686 字节（-1.92%）。图片内容和编码器版本会改变结果，因此项目锁死目标策略，不把常见的“13%/39%”说法当作合同。
