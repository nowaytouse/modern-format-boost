...
- JPEG→JXL 在所有元数据处理完成后，会先探测已安装官方 `djxl` 的真实能力：明确提供 `--reconstruct_jpeg` 的版本使用该操作；通过 `.jpg` 输出执行官方默认重建的版本使用其正式接口。两条路径都必须得到正向 JPEG 重建诊断、非空输出、无像素转 JPEG 回退以及逐字节哈希证明，不会因版本号或退出码猜测成功。JBRD、原始 Exif/XMP 与编码数据会被冻结；外部 XMP 以可重复执行且不改写原始层的 overlay 追加，并再次核验精确重建。`restore-jpeg` 不再改写重建出的 JPEG；额外 XMP 会作为单独哈希核验的同名 `.xmp` 侧车交付，因为嵌入它就不可能同时保持原 JPEG 字节完全一致。
