...
- **第五阶段 — 循环意图 (Loop Intent v3)**：采用全新的 7 层分层决策树模型。综合评估 **Loop Closure (循环闭合度)**、**Motion Gini (运动不均匀度)**、**周期性分析** 以及 **KNN 语义融合**，智能识别图像或视频的循环意图。
...
- `--force-video`: 强制将动图视为视频处理（忽略循环意图）。
...
这些格式本身已属于现代有损编码。二次编码会导致画质代际损伤 (Generational Loss)，程序默认会跳过以保护质量。
**例外情况**：如果检测到文件中包含 Apple/Google 高保真 **HDR Gainmap**，程序会将其合成输出为 JXL；或者当动图触发了 **循环意图** 优化机制时，也会进行相应处理。
