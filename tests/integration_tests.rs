...
mod integration_tests {
    ...
    fn test_media_files_are_readable() {
        ...
        for file in test_files {
            ...
            let metadata = std::fs::metadata(&path)
                .expect(&format!("Should read metadata for {}", file));
    ...
    fn test_media_directory_structure() {
        ...
        assert!(test_dir.join("videos").is_dir(), "videos/ directory required");
        assert!(test_dir.join("gifs").is_dir(), "gifs/ directory required");
        assert!(test_dir.join("MEDIA_MANIFEST.md").is_file(), "MEDIA_MANIFEST.md required");
    }
    ...
    fn test_ssim_quality_grades_video_coverage() {
        ...
        assert!(video_file("test_hq_source_15s.mp4").exists(), "High quality source");
        assert!(video_file("test_lq_source_12s.mp4").exists(), "Low quality source");
    }
    ...
    fn test_zero_gains_duration_coverage() {
        ...
        assert!(video_file("test_hq_source_15s.mp4").exists(), "15s duration");
    }
}
