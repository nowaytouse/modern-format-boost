...
mod numeric_cast_safety_tests {
    ...
    fn test_numeric_cast_returns_option_not_default() {
        ...
        let result_u64_to_u32: Option<u32> = numeric_cast::u64_to_u32_strict(large_u64, "overflow_test");
        // This MUST return None, never Some(0)
        assert!(result_u64_to_u32.is_none(), "Overflow must return None, not default value");
    }

    /// Verify f64_to_f32 lossy conversions don't hide precision loss
    #[test]
    ...
    fn test_integer_multiplication_overflow_safety() {
        ...
        let grid_size: u32 = 100_000; // Large but valid
        
        // SAFE pattern:
        let grid_size_sq = (grid_size as u64)
            .checked_mul(grid_size as u64)
    ...
    fn test_image_dimensions_no_hidden_arithmetic() {
        ...
        let total_pixels_safe = (width as u64)
    ...
    fn test_division_patterns_no_defaults() {
        ...
        let count = 0_usize;
        let safe_div = 100.0 / (count as f64).max(1.0);
        assert!(safe_div.is_finite());
        assert_eq!(safe_div, 100.0);
    }
}
