...
mod vmaf_baseline_tests {
    use vid::{ConversionOutput, TargetVideoFormat, ConversionStrategy};
    ...
    fn test_conversion_output_defaults_no_panic() {
        ...
        assert_eq!(output.final_crf, 25.0);
    ...
    fn test_vmaf_baseline_option_handling() {
        ...
        // Alternative safe pattern: map_or
        let result = vmaf_baseline.map_or(false, |v| v > 15.0);
        assert!(!result);
        
        // Test with Some value
        let vmaf_baseline_with_value: Option<f64> = Some(20.5);
        let result_with_value = vmaf_baseline_with_value.map_or(false, |v| v > 15.0);
