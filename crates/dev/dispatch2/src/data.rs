...
impl DispatchData {
    // Future: Expose this once possible in MSRV.
    ...
    pub fn from_static_bytes(data: &'static [u8]) -> DispatchRetained<Self> {
        ...
        let destructor = core::ptr::addr_of!(*NOOP_BLOCK).cast_mut();
    ...
    pub fn to_vec(&self) -> alloc::vec::Vec<u8> {
        ...
        // Future: Fix the need for this in `block2`.
        let block = unsafe {
            core::mem::transmute::<
                *mut block2::Block<
                    dyn Fn(NonNull<Self>, usize, NonNull<core::ffi::c_void>, usize) -> u8,
                >,
                *mut block2::Block<
                    dyn Fn(NonNull<Self>, usize, NonNull<core::ffi::c_void>, usize) -> bool,
                >,
            >(block)
        };
...
mod tests {
    ...
    fn concat() {
        ...
        assert_eq!(extended.to_vec(), b"foobarbar");
    }
