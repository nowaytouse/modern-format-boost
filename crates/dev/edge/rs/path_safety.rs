...
mod tests {
    ...
    fn test_sers_truncation_defense() {
        ...
        assert!(
            !safe_path.starts_with("file:"),
            "Should avoid file: protocol: {safe_path}"
        );
        assert!(
            !safe_path.starts_with("sers/"),
            "Found illegal path truncation: {safe_path}"
        );
    }
    ...
    fn test_format_expansion_prevention() {
        ...
        assert!(
            safe_path.contains("poison_pill_format_expansion.jpg"),
            "Failed to relativize: {safe_path}"
        );
    }
