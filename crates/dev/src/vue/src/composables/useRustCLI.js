    ...
    for (const p of possiblePaths) {
      try {
        ...
        if (output.includes('pixly-eagle-core')) {
          ...
          // 🔥 强制输出路径给用户看
          console.log('%c[Pixly Debug] Using Binary: ' + resolved, 'background: #222; color: #bada55; font-size: 14px');
  ...
  const convertImages = async (files, options, onProgress = null) => {
    ...
    try {
      ...
      for (let i = 0; i < mediaFiles.length; i++) {
        ...
        console.log('%c[Rust CLI Args] ' + JSON.stringify(args), 'color: #ff00ff')
        ...
        if (file.ext) {
          ...
          if (cleanInputFormat && cleanInputFormat.length > 0) {
            ...
            logger.debug(LOG_KEYS.RUST_CLI_EXEC, 'Using input format from Eagle metadata', {
              original: file.ext,
    ...
    try {
      ...
      if (fileExt) {
        ...
        if (cleanExt && cleanExt.length > 0) {
          ...
          logger.debug(LOG_KEYS.RUST_CLI_EXEC, 'Using format from Eagle metadata', {
            original: fileExt,
            cleaned: cleanExt
          })
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
          if (code !== 0) {
            logger.error(LOG_KEYS.RUST_CLI_ERROR, `❌ Analysis command failed with code ${code}`, {
              code,
              stderr,
              stdout
            })
          ...
          try {
            ...
            if (firstBrace === -1 || lastBrace === -1) {
              logger.warn(LOG_KEYS.RUST_CLI_ERROR, '⚠️ No JSON output from analyze command', {
            ...
            if (analysisData.optimization_status) {
              ...
              logger.info(LOG_KEYS.RUST_CLI_EXEC, '✅ Optimization status analyzed', {
                status: opt.status,
              ...
              logger.warn(LOG_KEYS.RUST_CLI_ERROR, '⚠️ No optimization_status in response', {
                analysisData
              })
              resolve(null)
            }
          } catch (error) {
            logger.error(LOG_KEYS.RUST_CLI_ERROR, '❌ Failed to parse analysis result', {
              error: error.message,
              stdout
            })
            reject(error)
        ...
        child.on('error', (error) => {
          logger.error(LOG_KEYS.RUST_CLI_ERROR, '❌ Failed to spawn analyze process', {
            error: error.message
          })
          reject(error)
        })
