...
const onDrop = (e: DragEvent) => {
  ...
  if (e.dataTransfer && e.dataTransfer.files.length > 0) {
    folderPath.value = e.dataTransfer.files[0].name || t('processing.dragged_folder');
    startMockProcessing();
  }
};
...
const selectFolder = async () => {
  ...
  dialogTimeout = setTimeout(() => {
    alert(t('dropzone.timeout') || 'Folder selection cancelled (no action for 30 minutes).');
...
const startCliProcessing = async () => {
  ...
  try {
    ...
    processing.value = false;
    logs.value.push('[INFO] Processing completed. You can close this window or process another folder.');
  }
};
...
const generateCliCommand = () => {
  ...
  const parentDir = targetPath.slice(0, Math.max(0, targetPath.lastIndexOf('/'))) || '/';
...
const onMouseMove = (e: MouseEvent) => {
  ...
  if (rafId === null) {
    rafId = requestAnimationFrame(() => {
      document.documentElement.style.setProperty('--mouse-x', `${String(currentMouseX)}px`);
      document.documentElement.style.setProperty('--mouse-y', `${String(currentMouseY)}px`);
      rafId = null;
    });
  }
};
...
onMounted(() => {
  ...
  invoke<string>('get_processor_binary_path')
    .then((path) => { processorBinaryPath.value = path; })
    .catch((error: unknown) => { console.error('Failed to get processor binary path:', error); });

  // Version Alignment Check Mechanism
  invoke('check_version_alignment')
    .then((message: unknown) => { console.log(message); })
    .catch((error: unknown) => {
    ...
    >
      <div class="cli-panel">
        ...
        <label class="cli-toggle-row">
          ...
          <div
            class="switch"
            :class="{ on: useExternalTerminal }"
          >
            <input
              v-model="useExternalTerminal"
              type="checkbox"
            >
        ...
        <div
          v-if="useExternalTerminal"
          class="external-terminal-section"
        >
    ...
    >
        ...
        <div class="panel-scroll">
          ...
          <div class="control-group toggles-group">
            <label class="toggle-row">
              <span class="toggle-text">{{ t('toggles.ultimateMode') }} <span class="badge warning">SLOW</span></span>
              <div
                class="switch"
                :class="{ on: mfbToggles.ultimateMode }"
              >
                <input
                  v-model="mfbToggles.ultimateMode"
                  type="checkbox"
                >
                <div class="switch-knob" />
              </div>
            ...
            <label class="toggle-row">
              ...
              <div
                class="switch"
                :class="{ on: mfbToggles.verboseMode }"
              >
                <input
                  v-model="mfbToggles.verboseMode"
                  type="checkbox"
                >
                <div class="switch-knob" />
              </div>
            ...
            <label class="toggle-row">
              ...
              <div
                class="switch"
                :class="{ on: mfbToggles.resumeMode }"
              >
                <input
                  v-model="mfbToggles.resumeMode"
                  type="checkbox"
                >
                <div class="switch-knob" />
              </div>
            ...
            <label class="toggle-row">
              <span class="toggle-text">{{ t('toggles.purgeCache') }} <span class="badge info">CACHE</span></span>
              <div
                class="switch"
                :class="{ on: mfbToggles.shortestPath }"
              >
                <input
                  v-model="mfbToggles.shortestPath"
                  type="checkbox"
                >
                <div class="switch-knob" />
              </div>
            ...
            <label class="toggle-row">
              ...
              <div
                class="switch"
                :class="{ on: mfbToggles.cleanOutput }"
              >
                <input
                  v-model="mfbToggles.cleanOutput"
                  type="checkbox"
                >
                <div class="switch-knob" />
              </div>
      ...
      >
        ...
        >
          ...
          >
            <div
              class="drop-icon-container"
              :class="{ float: isDragging }"
            >
...
.cli-panel {
  ...
  width: 700px; color: #00ff00; font-family: monospace;
  box-shadow: 0 10px 30px rgba(0,0,0,0.5);
  border: 1px solid #333;
  font-size: 15px;
}
.cli-panel h2 { margin-bottom: 20px; font-size: 1.5rem; text-transform: uppercase; border-bottom: 1px solid #00ff00; padding-bottom: 8px; }
...
.cli-toggle-row:hover { background: rgba(255,255,255,0.08); }
.toggle-label { font-size: 14px; font-weight: 600; color: #00ff00; }

.cli-divider {
  height: 1px; background: rgba(0,255,0,0.3); margin: 16px 0;
}

.cli-field { margin-bottom: 16px; display: flex; align-items: center; gap: 12px; }
.cli-field label { min-width: 80px; font-size: 14px; }
.cli-field select, .cli-field button { 
  background: #222; color: #00ff00; border: 1px solid #00ff00; 
  padding: 8px 14px; font-family: monospace; cursor: pointer; outline: none;
  font-size: 14px; border-radius: 4px; transition: all 0.2s;
}
.cli-field select:focus, .cli-field button:hover { background: #00ff00; color: #000; }
.cli-field button:disabled { border-color: #555; color: #555; background: #222; cursor: not-allowed; }
.cli-path { font-size: 13px; color: #aaa; word-break: break-all; flex: 1; }
.cli-start-btn { font-weight: bold; padding: 12px 24px !important; font-size: 15px; width: 100%; }

.cli-terminal {
  margin-top: 20px; background: #000; height: 250px; overflow-y: auto;
  padding: 10px; border: 1px solid #333; font-size: 13px; color: #ccc;
  border-radius: 4px;
}
.cli-log-line { border-bottom: 1px solid rgba(255,255,255,0.05); padding: 2px 0; white-space: pre-wrap; word-break: break-all; }

/* External Terminal Section */
.external-terminal-section { margin-top: 16px; }
.cli-buttons { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px; }
.cli-action-btn { 
  font-weight: bold; padding: 14px 20px; font-size: 15px; 
  background: #222; color: #00ff00; border: 1px solid #00ff00;
  border-radius: 6px; cursor: pointer; outline: none;
  transition: all 0.2s; font-family: monospace;
}
.cli-action-btn:hover:not(:disabled) { background: #00ff00; color: #000; transform: translateY(-2px); }
.cli-action-btn:active:not(:disabled) { transform: translateY(0); }
.cli-action-btn:disabled { border-color: #555; color: #555; background: #222; cursor: not-allowed; }
.cli-open-btn { background: #004400; border-color: #00ff00; }
.cli-open-btn:hover:not(:disabled) { background: #00aa00; color: #000; }

.cli-command-preview { 
  background: #000; border: 1px solid #333; border-radius: 6px; padding: 12px; 
  margin-bottom: 12px; overflow-x: auto;
}
.cli-command-preview pre { 
  margin: 0; color: #0f0; font-size: 13px; white-space: pre-wrap; word-break: break-all; 
}
.cli-hint { font-size: 13px; color: #888; text-align: center; margin: 0; font-style: italic; }
...
.ambient-bg {
  ...
  background-image: 
    radial-gradient(circle at 50% 20%, rgba(10, 132, 255, 0.15) 0%, transparent 40%),
    radial-gradient(circle at 80% 80%, rgba(191, 90, 242, 0.15) 0%, transparent 40%),
    radial-gradient(circle at 20% 80%, rgba(48, 209, 88, 0.1) 0%, transparent 40%);
...
.logo-icon { font-size: 1.5rem; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2)); }
.title-group h1 { font-size: 0.95rem; font-weight: 600; margin: 0; letter-spacing: -0.01em; }
.title-group p { font-size: 0.7rem; color: var(--text-muted); margin: 0; font-weight: 500; pointer-events: none;}
...
.icon-btn {
  ...
  transition: background 0.2s; font-size: 1rem; font-weight: 600;
}
.icon-btn:hover { background: rgba(128,128,128,0.15); }
.text-icon { font-size: 0.75rem; letter-spacing: 0.05em; }

/* Window Controls (Traffic Lights style) */
.window-controls { display: flex; gap: 8px; margin-left: 8px; }
.window-btn {
  width: 14px; height: 14px; border-radius: 50%; border: none;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; color: transparent; transition: all 0.2s;
}
.window-btn svg { width: 8px; height: 8px; }
.window-controls:hover .window-btn { color: rgba(0,0,0,0.5); }
.window-btn.close { background: #ff5f56; border: 1px solid #e0443e; }
.window-btn.minimize { background: #ffbd2e; border: 1px solid #dea123; }
.window-btn.maximize { background: #27c93f; border: 1px solid #1aab29; }
...
.panel-title { font-size: 1.1rem; font-weight: 600; display: flex; align-items: center; gap: 8px; margin:0;}
.panel-scroll {
...
.group-label { font-size: 0.75rem; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; }

.segmented-control {
  display: flex; background: rgba(0,0,0,0.15); border-radius: 10px; padding: 4px; gap: 4px;
}
...
.segment.active {
  ...
  box-shadow: 0 2px 8px rgba(0,0,0,0.1), inset 0 1px 1px var(--glass-highlight);
}
.seg-icon { font-size: 1.2rem; }
.seg-label { font-size: 0.7rem; font-weight: 500; text-align: center; }
...
[data-theme="light"] .liquid-select { background: rgba(0,0,0,0.05); }

/* Switch Toggles */
.toggles-group { background: rgba(0,0,0,0.1); border-radius: 12px; padding: 4px; }
[data-theme="light"] .toggles-group { background: rgba(0,0,0,0.03); }
.toggle-row {
  display: flex; align-items: center; justify-content: space-between;
  padding: 10px 12px; border-radius: 8px; cursor: pointer; transition: background 0.2s;
}
.toggle-row:hover { background: rgba(128,128,128,0.1); }
.toggle-text { font-size: 0.85rem; font-weight: 500; display: flex; align-items: center; gap: 8px; }
.badge { font-size: 0.6rem; padding: 2px 6px; border-radius: 4px; font-weight: 700; }
.badge.warning { background: rgba(255,149,0,0.2); color: var(--warning); }
.badge.info { background: rgba(10,132,255,0.2); color: var(--accent); }

.switch {
  width: 36px; height: 20px; background: rgba(128,128,128,0.3);
...
.panel-footer { margin-top: auto; padding-top: 16px; }
.liquid-btn {
  width: 100%; padding: 14px; border-radius: 12px; border: none;
  font-size: 0.95rem; font-weight: 600; cursor: pointer;
  background: var(--glass-bg); color: var(--text-main);
  box-shadow: inset 0 1px 1px var(--glass-highlight), 0 4px 12px rgba(0,0,0,0.1);
  border: 1px solid var(--glass-border); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
}
.liquid-btn.primary { background: var(--accent); color: #fff; border-color: var(--accent-hover); box-shadow: inset 0 1px 1px rgba(255,255,255,0.3), 0 4px 15px rgba(10,132,255,0.3); }
.liquid-btn.primary:hover:not(:disabled) { background: var(--accent-hover); transform: translateY(-1px); }
.liquid-btn.primary:active:not(:disabled) { transform: translateY(1px) scale(0.97); box-shadow: inset 0 2px 4px rgba(0,0,0,0.2), 0 2px 8px rgba(10,132,255,0.2); }
...
.drag-glow {
  position: absolute; inset: 0; background: rgba(10, 132, 255, 0.1);
  pointer-events: none; transition: opacity 0.3s;
}

.drop-idle, .drop-processing {
  display: flex; flex-direction: column; align-items: center; text-align: center; z-index: 2;
}

.drop-icon-container { position: relative; margin-bottom: 20px; transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1); }
.drop-icon-container.float { transform: translateY(-20px) scale(1.1); }
.huge-icon { font-size: 5rem; filter: drop-shadow(0 10px 20px rgba(0,0,0,0.2)); position: relative; z-index: 2; }
.ripple-ring {
  position: absolute; inset: -20px; border-radius: 50%;
  border: 2px dashed var(--glass-border); animation: spin-slow 10s linear infinite;
}
@keyframes spin-slow { 100% { transform: rotate(360deg); } }

.drop-title { font-size: 1.8rem; font-weight: 700; margin-bottom: 8px; letter-spacing: -0.02em; }
.drop-subtitle { font-size: 0.95rem; color: var(--text-muted); max-width: 300px; line-height: 1.4; }

/* Processing UI */
.progress-ring { position: relative; display: flex; align-items: center; justify-content: center; margin-bottom: 24px; }
.ring-bg { fill: none; stroke: rgba(128,128,128,0.1); stroke-width: 6; }
.ring-fill { fill: none; stroke: var(--accent); stroke-width: 6; stroke-linecap: round; transform: rotate(-90deg); transform-origin: 50% 50%; transition: stroke-dashoffset 0.1s linear; }
.progress-text { position: absolute; font-size: 2rem; font-weight: 700; }

.proc-details { display: flex; flex-direction: column; align-items: center; gap: 12px; }
.proc-target { font-size: 1.2rem; font-weight: 600; }
.pulse-bar { width: 150px; height: 4px; background: rgba(128,128,128,0.2); border-radius: 2px; overflow: hidden; position: relative; }
.pulse-indicator { position: absolute; top:0; left:0; height:100%; width: 40%; background: var(--accent); border-radius: 2px; animation: sweep 1.5s infinite ease-in-out; }
@keyframes sweep {
  0% { transform: translateX(-100%); }
  100% { transform: translateX(300%); }
}
.proc-status { font-size: 0.85rem; color: var(--text-muted); }
