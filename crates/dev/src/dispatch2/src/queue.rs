...
use crate::{
    numeric_cast, DispatchObject, DispatchQoS, DispatchRetained, DispatchTime,
...
enum_with_val! {
    ...
    pub struct DispatchQueueGlobalPriority(pub c_long) {
        ...
        Background = numeric_cast::i16_to_c_long(i16::MIN),
    }
}
...
impl GlobalQueueIdentifier {
    ...
    #[inline]
    #[must_use]
    pub const fn to_identifier(self) -> isize {
        match self {
            Self::Priority(queue_priority) => numeric_cast::c_long_to_isize(queue_priority.0),
            Self::QualityOfService(qos_class) => numeric_cast::u32_to_isize(qos_class.0),
        }
    }
}
...
impl DispatchQueue {
    ...
    // Future: Mark this as `const` once in MSRV.
    #[inline]
    ...
    /// Apply blocks automatically in parallel when using `dispatch_apply`.
    #[doc(alias = "DISPATCH_APPLY_AUTO")]
    pub const APPLY_AUTO: Option<&Self> = None;

    /// Use the default target queue for an object type.
    #[doc(alias = "DISPATCH_TARGET_QUEUE_DEFAULT")]
    pub const TARGET_QUEUE_DEFAULT: Option<&Self> = None;

    /// Sentinel for [`DispatchQueue::get_label`] when called on the current queue.
    #[doc(alias = "DISPATCH_CURRENT_QUEUE_LABEL")]
    pub const CURRENT_QUEUE_LABEL: Option<&Self> = None;
}
...
pub fn dispatch_main() -> ! {
    ...
    // SAFETY: Future: Must this be run on the main thread? Do we need to take
    // `MainThreadMarker`?
