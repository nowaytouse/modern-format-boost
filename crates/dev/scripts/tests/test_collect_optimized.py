...
import collect_optimized  # noqa: E402
...
def test_snapshot_directories_fails_closed_on_stat_error(tmp_path):
    ...
    with patch.object(
        ...
        with pytest.raises(
            collect_optimized.CollectMetadataError, match="snapshot failed"
        ):
...
def test_restore_directory_times_fails_closed_on_utime_error(tmp_path):
    ...
    with patch.object(
        ...
        with pytest.raises(
            collect_optimized.CollectMetadataError, match="restore failed"
        ):
