...
References the complete Rust implementation in crates/foundation/src/xmp_merger.rs
"""
...
def _warn(msg: str):
    print(
        f"{YELLOW}{pick_symbol('⚠️', '[WARN]')} {msg}{RESET}",
        file=sys.stderr,
    )
...
def check_exiftool():
    if not shutil.which("exiftool"):
        print(
            f"{RED}{pick_symbol('❌', ('[ERROR]'))} ExifTool not found. Please install it first: brew install exiftool{RESET}"
        )
        sys.exit(1)
...
def extract_xmp_metadata(xmp_path: Path) -> XmpInfo:
    ...
    try:
        ...
        _warn(f"extract_xmp_metadata failed for {xmp_path}: {exc}")
        return XmpInfo()
...
def extract_media_doc_id(media_path: Path) -> str:
    ...
    try:
        ...
        _warn(f"extract_media_doc_id failed for {media_path}: {exc}")
        return ""
...
def scan_xmp_ref(media_path: Path, target_xmp: str) -> bool:
    ...
    try:
        ...
        _warn(f"scan_xmp_ref failed for {media_path}: {exc}")
        return False
...
def extract_batch_doc_ids(media_paths: list[Path]) -> dict[str, str]:
    ...
    try:
        ...
        _warn(f"extract_batch_doc_ids failed: {exc}")
        return {}
...
def find_media_match(xmp_path: Path) -> tuple[Path | None, str]:
    ...
    if xmp_info.doc_id and is_uuid_format(xmp_stem):
        print(
            f"  {pick_symbol('🔍', ('[SEARCH]'))} Searching by DocumentID: {xmp_info.doc_id}"
        )
    ...
    for p in parent.rglob("*"):
        ...
        try:
            ...
            _warn(f"skip depth check for {p}: {exc}")
            continue
...
def get_mac_time_attr(path: str, attr_flag: int) -> TimeSpec | None:
    ...
    try:
        ...
        _warn(f"get_mac_time_attr failed for {path}: {exc}")
        return None
...
def set_mac_time_attr(path: str, attr_flag: int, timespec: TimeSpec):
    ...
    try:
        ...
        _warn(f"set_mac_time_attr failed for {path}: {exc}")
...
def get_timestamps(path: Path) -> FileTimestamps | None:
    try:
        ...
        _warn(f"get_timestamps failed for {path}: {exc}")
        return None
...
def restore_timestamps(path: Path, ts: FileTimestamps | None):
    ...
    try:
        ...
        print(
            f"  {YELLOW}{pick_symbol('⚠️', '[WARN]')}  Failed to restore timestamps for {path.name}: {e}{RESET}"
        )
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            if is_real_error and "[minor]" not in err_msg.lower():
                print(
                    f"  {RED}{pick_symbol('❌', ('[ERROR]'))} Failed: {err_msg}{RESET}"
                )
                return False
        ...
        try:
            ...
            print(
                f"  {YELLOW}{pick_symbol('⚠️', '[WARN]')}  XMP merge succeeded but sidecar delete failed: {e}{RESET}"
            )

        print(f"  {GREEN}{pick_symbol('✅', ('[OK]'))} Success (XMP deleted){RESET}")
        return True
    except (
        OSError,
        ValueError,
        ...
        print(
            f"  {RED}{pick_symbol('❌', ('[ERROR]'))} Error executing exiftool: {e}{RESET}"
        )
        return False
...
def main():
    if len(sys.argv) > 1:
        ...
        print(
            f"{RED}{pick_symbol('❌', ('[ERROR]'))} Error: Please provide a target directory.{RESET}"
        )
    ...
    if not target_dir.is_dir():
        print(
            f"{RED}{pick_symbol('❌', ('[ERROR]'))} Directory not found: {target_dir}{RESET}"
        )
        sys.exit(1)
    ...
    for xmp_path in xmp_files:
        ...
        if media_path:
            ...
            print(
                f"  {YELLOW}{pick_symbol('⚠️', '[WARN]')}  Skipped ({strategy}):{RESET} {xmp_path.name}"
            )
            skip_count += 1
