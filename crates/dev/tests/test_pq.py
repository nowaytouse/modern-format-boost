...
def read_exr(filepath):
    ...
    img[:, :, 0] = np.frombuffer(channels[0], dtype=np.float32).reshape(size[1], size[0])
    img[:, :, 1] = np.frombuffer(channels[1], dtype=np.float32).reshape(size[1], size[0])
    img[:, :, 2] = np.frombuffer(channels[2], dtype=np.float32).reshape(size[1], size[0])
    return img
