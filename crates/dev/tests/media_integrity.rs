...
mod tests {
    ...
    fn test_zero_byte_file_handling() {
        ...
        assert!(result.is_err(), "Pipeline should fail gracefully for 0-byte files!");
        
        let (_, _, error_msg) = result.unwrap_err();
        // Since ImageMagick is killed or fails, there might be no stdout, but it must not panic
        assert!(error_msg.is_empty() || error_msg.contains("failed") || error_msg.contains("not available"));
    }
    ...
    fn test_trailing_space_path_loading() {
        ...
        assert!(result.is_err(), "Expected failure for synthetic space-padded file");
    }
    ...
    fn test_metadata_bomb_stamina() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, Path::new("bomb.jxl"), 1.0, 1, false, 8, false, false, 7
        );
        
        // Success here means no OOM and no hang during metadata reading/piping
        assert!(result.is_ok() || result.is_err(), "Metadata bomb caused a crash or infinite hang!");
    }
}
