...
mod tests {
    ...
    fn test_grayscale_icc_fallback() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, output, 1.0, 1, false, 8, false, false, 7
        );
    ...
    fn test_single_frame_veto() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, Path::new("dummy.jxl"), 1.0, 1, false, 8, false, false, 7
        );
        
        // The result should indicate success or a clean handled failure if it's not a real multi-frame
        assert!(result.is_ok() || result.is_err(), "Static veto test crashed!");
    }
    ...
    fn test_vfr_fps_calculation() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, Path::new("rhythm.jxl"), 1.0, 1, false, 8, false, false, 7
        );
        
        assert!(result.is_ok() || result.is_err(), "VFR FPS calculation test crashed!");
    }
    ...
    fn test_non_monotonic_pts_fallback() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, Path::new("pts.jxl"), 1.0, 1, false, 8, false, false, 7
        );
        
        assert!(result.is_ok() || result.is_err(), "Broken PTS fallback test crashed!");
    }
    ...
    fn test_bpp_calculation_precision() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, Path::new("bpp_test.jxl"), 1.0, 1, false, 8, false, false, 7
        );
    ...
    fn test_zero_duration_rhythm_interception() {
        ...
        let result = run_imagemagick_cjxl_pipeline(
            input, Path::new("zero.jxl"), 1.0, 1, false, 8, false, false, 7
        );
        
        // This validates the FPS logic handles or rejects invalid timing
        assert!(result.is_err(), "Pipeline should reject zero-duration playback!");
    }
}
