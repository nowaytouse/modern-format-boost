...
fn map_legacy_rel(rel: &str) -> String {
    for (old, new) in [
        (
            "crates/dev/tests/comprehensive_weakness_audit.rs",
            "crates/dev/tests/contract/comprehensive_weakness_audit.rs",
        ),
        (
            "crates/dev/tests/numeric_cast_safety.rs",
            "crates/dev/tests/contract/numeric_cast_safety.rs",
        ),
        (
            "crates/dev/tests/test_real_silent_fallbacks.rs",
            "crates/dev/tests/contract/test_real_silent_fallbacks.rs",
        ),
        (
            "crates/dev/tests/test_silent_numeric_fallbacks.rs",
            "crates/dev/tests/contract/test_silent_numeric_fallbacks.rs",
        ),
        (
            "crates/dev/tests/classification_snapshots.rs",
            "crates/dev/tests/matrix/classification_snapshots.rs",
        ),
        (
            "crates/dev/tests/mode_matrix_tests.rs",
            "crates/dev/tests/matrix/mode_matrix_tests.rs",
        ),
        (
            "crates/dev/tests/parity_tests.rs",
            "crates/dev/tests/matrix/parity_tests.rs",
        ),
        (
            "crates/dev/tests/property_tests.rs",
            "crates/dev/tests/matrix/property_tests.rs",
        ),
        (
            "crates/dev/tests/snapshot_tests.rs",
            "crates/dev/tests/matrix/snapshot_tests.rs",
        ),
        (
            "crates/dev/tests/deny_animated_jxl.rs",
            "crates/dev/tests/media/deny_animated_jxl.rs",
        ),
        (
            "crates/dev/tests/headless_gif_regression.rs",
            "crates/dev/tests/media/headless_gif_regression.rs",
        ),
        (
            "crates/dev/tests/test_animated_frame_consistency.rs",
            "crates/dev/tests/media/test_animated_frame_consistency.rs",
        ),
        (
            "crates/dev/tests/test_cjxl_errors.rs",
            "crates/dev/tests/media/test_cjxl_errors.rs",
        ),
        (
            "crates/dev/tests/test_ultrahdr_hardening.rs",
            "crates/dev/tests/media/test_ultrahdr_hardening.rs",
        ),
        (
            "crates/dev/tests/test_webp_animated_classification.rs",
            "crates/dev/tests/media/test_webp_animated_classification.rs",
        ),
        (
            "crates/dev/tests/test_webp_duration_parser.rs",
            "crates/dev/tests/media/test_webp_duration_parser.rs",
        ),
        (
            "crates/dev/tests/vmaf_baseline_missing.rs",
            "crates/dev/tests/media/vmaf_baseline_missing.rs",
        ),
        (
            "crates/dev/tests/ctrlc_behavior.rs",
            "crates/dev/tests/runtime/ctrlc_behavior.rs",
        ),
        (
            "crates/dev/tests/ignored_semantics.rs",
            "crates/dev/tests/runtime/ignored_semantics.rs",
        ),
        (
            "crates/dev/tests/ignored_static.rs",
            "crates/dev/tests/runtime/ignored_static.rs",
        ),
        (
            "crates/dev/tests/path_safety.rs",
            "crates/dev/tests/runtime/path_safety.rs",
        ),
        (
            "crates/dev/tests/runtime_probe_regression.rs",
            "crates/dev/tests/runtime/runtime_probe_regression.rs",
        ),
    ] {
        if rel == old {
...
fn workspace_fuzz_rust_files(root: &Path) -> Vec<PathBuf> {
    let fuzz_dir = root.join("crates/dev/fuzz/fuzz_targets");
    let mut files = Vec::new();
    if !fuzz_dir.is_dir() {
        return files;
    }
...
fn workspace_whole_repo_rust_production_files(root: &Path) -> Vec<PathBuf> {
    ...
    files.extend(workspace_crate_production_rust_files(
        root,
        &["dev/dispatch2"],
    ));
...
/// `crates/dev/tests/**/*.rs` excluding pattern-catalog harness files (M240
/// whole-repo parity).
fn workspace_dev_integration_test_rust_files(root: &Path) -> Vec<PathBuf> {
    let tests_dir = root.join("crates/dev/tests");
    let mut files = Vec::new();
    if tests_dir.is_dir() {
...
fn media_conversion_unified_fabrication_patterns_m243_extension() {
    ...
    for fixture in [
        ...
        let content = fs::read_to_string(root.join("crates/dev/tests").join(fixture))
...
fn media_conversion_synthetic_edge_fixture_closure_m244() {
    ...
    let webp = fs::read_to_string(join_legacy_aware(
        &root,
        "crates/dev/tests/edge/gifs/synth_webp.rs",
    ))
...
fn delivery_metadata_contract_artifacts_locked() {
    let root = workspace_root();
    for rel in [
        "crates/foundation/tests/internal/exif_structural_repair_contract.rs",
        "crates/foundation/tests/internal/xmp_jxl_apple_compat_contract.rs",
        "crates/foundation/tests/internal/metadata_preservation_contract.rs",
...
fn media_conversion_webp_header_preflight_m123() {
    ...
    let tests = fs::read_to_string(join_legacy_aware(
        &root,
        "crates/foundation/tests/internal/video_detection.rs",
    ))
...
fn media_conversion_conversion_cwd_m168() {
    ...
    assert!(
        prod_common.contains("get_mfb_root") && prod_common.contains("delivery_temp_mfb_root_ssot"),
        "common_utils cache paths must use the central MFB root and system-temp fallback (M168)"
    );
    assert!(
...
fn obsolete_blocking_exit_guard_is_not_present() {
    ...
    assert!(
        !root
            .join("crates/.modern_format_boost/.tmp_lib/libstdc++.tbd")
            .exists(),
