//! SMOKE TESTS: Basic sanity checks for media processing flows.
//! These tests use tiny synthetic assets to verify basic I/O and header detection.
//! They do not perform deep forensic validation or stress real processing pipelines.
...
mod test_utils {
    ...
    pub(super) fn create_test_jpeg() -> Vec<u8> {
    ...
    pub(super) fn create_test_png() -> Vec<u8> {
        let mut png = Vec::new();
    ...
    pub(super) fn create_test_webp() -> Vec<u8> {
    ...
    pub(super) fn create_static_gif() -> Vec<u8> {
        let mut gif = Vec::new();
    ...
    pub(super) fn create_animated_gif() -> Vec<u8> {
        let mut gif = Vec::new();
    ...
    pub(super) fn create_animated_webp() -> Vec<u8> {
        let mut webp = Vec::new();
        // RIFF header
        webp.extend_from_slice(b"RIFF");
    ...
    pub(super) fn create_test_mp4() -> Vec<u8> {
        let mut mp4 = Vec::new();
        // ftyp box
    ...
    pub(super) fn write_test_file<P: AsRef<Path>>(data: &[u8], path: P) -> Result<()> {
...
#[test]
fn smoke_jpeg_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_png_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_webp_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_animated_webp_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_static_gif_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_animated_gif_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_mp4_processing_flow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_complete_media_processing_workflow() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_error_handling_and_recovery() -> Result<()> {
    use test_utils::*;
...
#[test]
fn smoke_performance_and_memory_safety() -> Result<()> {
    use test_utils::*;
...
mod tests {
    ...
    fn run_all_media_flow_tests() -> Result<()> {
        ...
        // Image tests
        smoke_jpeg_processing_flow()?;
        smoke_png_processing_flow()?;
        smoke_webp_processing_flow()?;

        // Animated image tests
        smoke_static_gif_processing_flow()?;
        smoke_animated_gif_processing_flow()?;

        // Video tests
        smoke_mp4_processing_flow()?;

        // Integration tests
        smoke_complete_media_processing_workflow()?;
        smoke_error_handling_and_recovery()?;
        smoke_performance_and_memory_safety()?;
