...
#[inline]
pub fn compare_pass_gate(left_passed: bool, right_passed: bool) -> Ordering {
...
#[inline]
pub fn compare_pass_gate_result<E>(left: &Result<(), E>, right: &Result<(), E>) -> Ordering {
...
mod tests {
    ...
    fn test_compare_quality_desc() {
        ...
        assert_eq!(
            compare_quality_desc(Some(0.90), Some(0.95), 1e-4),
            Ordering::Greater
        );

        // Within epsilon is equal
        assert_eq!(
            compare_quality_desc(Some(0.95), Some(0.95 + 1e-5), 1e-4),
            Ordering::Equal
        );
