...
mod tests {
    ...
    fn test_skip_vs_error_semantics() {
        ...
        assert!(
            iter_err.should_copy_original(),
            "IterationLimitExceeded must trigger original copy"
        );
        ...
        assert!(
            qual_err.should_copy_original(),
            "QualityValidationFailed must trigger original copy"
        );
        ...
        assert!(
            !anal_err.should_copy_original(),
            "AnalysisError must NOT trigger original copy"
        );
        ...
        assert!(
            !io_err.is_skip(),
            "VideoFormatNotSupported must NOT be a skip"
        );
        assert!(
            !io_err.should_copy_original(),
            "VideoFormatNotSupported must NOT trigger original copy"
        );
