-- V7 Seed Samples - Boundary Logic Guided Dataset
-- These samples provide the first "cognitive boundaries" for the KNN engine during database initialization.
-- Includes: minimal memes, high-frame stickers, high-value art loops, and distractors (short videos).
...
-- 1. Classic meme boundary (LoopStrong): short, fast, small size, low entropy
(
...
-- 2. Transparent sticker boundary (LoopStrong): has transparency, usually very short action
(
...
-- 3. High-value art loop (LoopStrong): high resolution, has color profile (ICC), art directory
(
...
-- 4. Distractor boundary: looks like loop but is actually short video (LoopWeak)
(
    'seed_hash_004_short_video', '/seed/clips/garden_record.mp4', 'garden.mp4', 'mp4',
    1920, 1080, 4.5, 135, 8000000, 30.0,
    0.08, 1.2, false, false,
    false, false, false, false,
    'video', 'LoopWeak', 'seed_v7', 1.777, 2073600
),

-- 5. Long video boundary (LoopWeak): duration exceeds threshold, high complexity, not native GIF
(
    'seed_hash_005_long_content', '/seed/videos/vlog_segment.mov', 'vlog_01.mov', 'mov',
    1280, 720, 25.0, 750, 45000000, 30.0,
    0.15, 0.8, false, false,
    false, false, false, false,
    'video', 'LoopWeak', 'seed_v7', 1.777, 921600
),

-- 6. Extreme high-frequency loop (LoopStrong): very high frame rate but very short duration, common in high-quality animated stickers
(
...
-- 14. Security camera feed (LoopWeak): grayscale, noisy, low frame rate, very long
(
...
-- 15. AI generated hallucination loop (LoopStrong): flickering images, high-frequency changes, but logically looping
(
    'seed_hash_015_ai_dream', '/seed/ai/mandelbrot.mp4', 'ai_dream.mp4', 'mp4',
    1024, 1024, 4.0, 120, 10000000, 30.0,
    0.08, 2.4, false, false,
    false, true, false, true,
    'low', 'LoopStrong', 'seed_v7', 1.0, 1048576
),

-- 16. Retro VFR animation (LoopStrong): unstable frame rate, manual creation traces
(
    'seed_hash_016_vfr_retro', '/seed/retro/pixel_dance.gif', 'dance.gif', 'gif',
    100, 100, 2.5, 12, 150000, 4.8,
    0.12, 4.8, false, false,
    true, true, true, false,
    'high', 'LoopStrong', 'seed_v7', 1.0, 10000
),

-- 17. Cinemagraph static photo with local motion (LoopStrong): mostly static, local loop
(
    'seed_hash_017_cinemagraph', '/seed/gallery/coffee_steam.gif', 'coffee.gif', 'gif',
    1920, 1080, 10.0, 240, 25000000, 24.0,
    0.04, 1.8, false, false,
    false, true, false, true,
    'low', 'LoopStrong', 'seed_v7', 1.777, 2073600
),

-- 18. Very long silent loop (LoopStrong): even 20s is loop intent
(
...
-- 19. Error collected web background (LoopStrong): very small, tiled, solid color changes
(
