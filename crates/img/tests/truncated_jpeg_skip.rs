...
#[test]
fn truncated_jpeg_is_skipped_with_delivery_required() {
    ...
    foundation::init_ghost_mode().expect("init ghost mode");

    // Create a truncated JPEG (only SOI + APP0 marker bytes)
    let input = tmp_dir.path().join("truncated.jpg");
    fs::write(&input, &[0xFFu8, 0xD8u8, 0xFFu8, 0xE0u8]).expect("write truncated jpeg");
    ...
    let result = convert_jpeg_to_jxl(&input, &options, None)
        .expect("convert_jpeg_to_jxl should return Ok(TaskResult) for truncated input");
