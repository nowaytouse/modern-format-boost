    ...
    let temp_icc = match crate::media_conversion_gate::delivery_named_tempfile_in_scratch_or_err(
        ...
        Err(e) => {
            crate::media_conversion_gate::delivery_jxl_path_audit(
                ...
                src,
                format!(
                    "JXL METADATA AUDIT: Failed to create temporary file for ICC extraction from '{}' | System Error: {}",
                    src.display(),
                    e
                ),
            );
        ...
        .output()
    {
        ...
        Err(e) => {
            crate::media_conversion_gate::delivery_jxl_path_audit(
                ...
                src,
                format!(
                    "JXL METADATA AUDIT: Failed to execute ExifTool for ICC extraction from '{}' | Pipeline Error: {}",
                    src.display(),
                    e
                ),
            );
...
///
/// Returns `Ok(None)` when ICC extraction is honestly unavailable or produces no ICC payload.
///
/// # Errors
///
/// Returns an error when temporary file creation, ICC extraction, or persistence of the
/// patched profile fails after extraction was attempted.
...
pub fn verify_jxl_health(path: &Path) -> Result<(), String> {
    ...
    if sig != [0xFF, 0x0A] && sig != [0x00, 0x00] {
        crate::media_conversion_gate::delivery_jxl_batch_fallback_audit(
            "delivery_jxl",
            format!(
                "JXL AUDIT: Invalid signature | Forensic: Found {:02X}{:02X}; expected FF0A or 0000; refusing to parse non-JXL data",
                sig[0], sig[1]
            ),
        );
        ...
        match result {
            Ok(output) if !output.status.success() => {
                ...
                crate::media_conversion_gate::delivery_jxl_batch_fallback_audit(
                    "delivery_jxl",
                    format!(
                        "JXL AUDIT: Health check failed | Forensic: jxlinfo returned non-zero exit; stderr: '{trimmed}'; bitstream is likely corrupt"
                    ),
                );
            ...
            Err(err) => {
                crate::media_conversion_gate::delivery_jxl_batch_fallback_audit(
                    "delivery_jxl",
                    format!(
                        "JXL AUDIT: Health check failed | Forensic: failed to execute jxlinfo after availability check: {err}"
                    ),
                );
...
/// Tool absence returns `Ok(false)` so callers can inject ICC as a fallback; probe execution
/// failures return `Err` so they are not confused with an ICC-absent JXL.
...
/// retry with a `.jpg` temp output instead of treating the bitstream as corrupt.
#[must_use]
...
pub fn get_png_bit_depth(path: &Path) -> Option<u8> {
    ...
    let mut f = match std::fs::File::open(path) {
        ...
        Err(e) => {
            crate::media_conversion_gate::delivery_jxl_path_audit(
                ...
                path,
                format!(
                    "PNG BIT-DEPTH PROBE: Failed to open file for analysis at '{}' | System Error: {}",
                    path.display(),
                    e
                ),
            );
    ...
    if let Err(e) = f.read_exact(&mut buf) {
        crate::media_conversion_gate::delivery_jxl_path_audit(
            ...
            path,
            format!(
                "PNG BIT-DEPTH PROBE: Failed to read IHDR header from '{}' (insufficient bytes) | I/O Error: {}",
                path.display(),
                e
            ),
        );
    ...
    if &buf[0..8] != b"\x89PNG\r\n\x1a\n" {
        crate::media_conversion_gate::delivery_jxl_path_audit(
            ...
            path,
            format!(
                "PNG BIT-DEPTH PROBE: Invalid PNG signature detected at '{}' | Forensic: Mismatched magic bytes",
                path.display()
            ),
        );
        return None;
    ...
    if buf[8..12] != [0, 0, 0, 13] || &buf[12..16] != b"IHDR" {
        crate::media_conversion_gate::delivery_jxl_path_audit(
            ...
            path,
            format!(
                "PNG BIT-DEPTH PROBE: Missing canonical IHDR header at '{}' | Preserving unknown precision instead of trusting malformed header layout",
                path.display()
            ),
        );
        return None;
    ...
    if matches!(bit_depth, 1 | 2 | 4 | 8 | 16) {
        ...
        crate::media_conversion_gate::delivery_jxl_path_audit(
            ...
            path,
            format!(
                "PNG BIT-DEPTH PROBE: Invalid IHDR bit depth {} detected at '{}' | Preserving unknown precision instead of forging retry depth",
                bit_depth,
                path.display()
            ),
        );
    ...
    match classify_png_eight_bit_retry(get_png_bit_depth(input)) {
        ...
        PngEightBitRetryDisposition::Unknown => {
            crate::media_conversion_gate::delivery_jxl_path_audit(
                ...
                input,
                format!(
                    "JXL CONVERSION AUDIT: Failed to detect PNG bit depth for '{}' | Preserving unknown precision; refusing speculative 8-bit retry",
                    input.display()
                ),
            );
            crate::progress_mode::emit_stderr(&format!(
...
///   - still fails + decode/pixel error + confirmed 8-bit source → -strip, depth 8 (no quality loss)
///   - still fails + higher/unknown bit depth → normalize ICC to sRGB, keep safety depth
///     - still fails → error, refuse to downgrade
/// - decode/pixel error + confirmed 8-bit source → -strip, depth 8 (no quality loss)
/// - decode/pixel error + higher/unknown bit depth → normalize ICC to sRGB, keep safety depth
    ...
    match run_imagemagick_cjxl_pipeline_with_effort(base_request) {
        ...
        Err((magick_ok, cjxl_ok, stderr)) => {
            ...
            if magick_ok && !cjxl_ok && is_grayscale_icc_cjxl_error(&stderr) {
                ...
                match run_imagemagick_cjxl_pipeline_with_effort(ImagemagickCjxlPipelineRequest {
                    ...
                    Err((m, c, stderr2)) => {
                        ...
                        if m && !c && is_decode_or_pixel_cjxl_error(&stderr2) {
                            ...
                            // Attempt 3: normalize ICC to sRGB, preserve safety depth when 8-bit was not confirmed.
                            crate::progress_mode::emit_stderr(&format!(
                                "   {} Attempt 3: ICC normalization (sRGB, higher/unknown bit depth)",
                                crate::modern_ui::symbols::styled_retry_icon(),
                            ));
                ...
                // Attempt 2: normalize ICC to sRGB, preserve safety depth when 8-bit was not confirmed.
                crate::progress_mode::emit_stderr(&format!(
            ...
            if magick_ok && !cjxl_ok && !stderr.contains("-strip") {
                ...
                match run_imagemagick_cjxl_pipeline_with_effort(ImagemagickCjxlPipelineRequest {
                    ...
                    Err((retry_magick_ok, retry_cjxl_ok, retry_stderr)) => {
                        crate::progress_mode::emit_stderr(&format!(
                            "   {} Final attempt failed (magick_ok={retry_magick_ok}, cjxl_ok={retry_cjxl_ok}): {}",
                            crate::modern_ui::symbols::styled_ok_fail_label(false),
            ...
            if magick_ok && !cjxl_ok && is_cjxl_signal_killed(&stderr) {
                ...
                match run_imagemagick_cjxl_pipeline_with_effort(ImagemagickCjxlPipelineRequest {
                    ...
                    Err((retry_magick_ok, retry_cjxl_ok, retry_stderr)) => {
                        crate::progress_mode::emit_stderr(&format!(
                            "   {} Signal-kill retry failed (magick_ok={retry_magick_ok}, cjxl_ok={retry_cjxl_ok}): {}",
                            crate::modern_ui::symbols::styled_ok_fail_label(false),
                            crate::io_utils::tail_error_lines(&retry_stderr, 2)
                        ));
                    }
