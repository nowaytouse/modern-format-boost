...
pub fn calculate_psnr(original: &DynamicImage, converted: &DynamicImage) -> Option<f64> {
    ...
    let orig_pixels: Vec<_> = orig_rgb.pixels().iter().collect();
    let conv_pixels: Vec<_> = conv_rgb.pixels().iter().collect();
    ...
    let mse = if cfg!(feature = "high-precision") && !cfg!(feature = "ci-static-build") {
        #[cfg(feature = "high-precision")]
        {
            ...
            let mse_sum_u64 =
                match crate::numeric_cast::f64_to_u64_strict(mse_sum.round(), "mse_sum") {
                    Some(v) => v,
                    None => unreachable!(
                        "CRITICAL: MSE sum overflow/NaN in calculate_psnr (mse_sum={})",
                        mse_sum
                    ),
                };
            let mse_sum_int = Integer::from(mse_sum_u64);
...
pub fn calculate_ssim(original: &DynamicImage, converted: &DynamicImage) -> Option<f64> {
    ...
    let count_r = match Rational::from_f64(count) {
        Some(v) => v,
        None => unreachable!(
            "CRITICAL: positions.len() is non-finite in calculate_ssim (count={})",
            count
        ),
    };
...
fn calculate_window_ssim(
    ...
    for (i, row) in window.iter().enumerate() {
        for (j, _) in row.iter().enumerate() {
            ...
            let pixel_x = match crate::numeric_cast::usize_to_u32_strict(px, "px") {
                Some(v) => v,
                None => unreachable!(
                    "CRITICAL: px overflow in calculate_window_ssim (px={}, x={}, j={})",
                    px, x, j
                ),
            };
            let pixel_y = match crate::numeric_cast::usize_to_u32_strict(py, "py") {
                Some(v) => v,
                None => unreachable!(
                    "CRITICAL: py overflow in calculate_window_ssim (py={}, y={}, i={})",
                    py, y, i
                ),
            };
    ...
    let denominator = (mean_x.mul_add(mean_x, mean_y * mean_y) + C1) * (var_x + var_y + C2);
...
fn calculate_ssim_simple(original: &DynamicImage, converted: &DynamicImage) -> Option<f64> {
    ...
    for (p_orig, p_conv) in orig_gray.pixels().iter().zip(conv_gray.pixels()) {
    ...
    #[cfg(feature = "high-precision")]
    {
        ...
        let c1_rat = match Rational::from_f64(C1) {
            Some(v) => v,
            None => unreachable!("CRITICAL: C1 stability constant is non-finite (C1={})", C1),
        };
        let c2_rat = match Rational::from_f64(C2) {
            Some(v) => v,
            None => unreachable!("CRITICAL: C2 stability constant is non-finite (C2={})", C2),
        };
        ...
        if denominator == 0 {
            return Some(crate::constants::SSIM_DEGENERATE_MATCH_VALUE);
        }
...
pub fn calculate_ms_ssim(original: &DynamicImage, converted: &DynamicImage) -> Option<f64> {
    ...
    for (i, &weight) in weights.iter().enumerate().take(scales) {
        ...
        let window_u32 = match crate::numeric_cast::usize_to_u32_strict(WINDOW_SIZE, "window_size")
        {
            Some(v) => v,
            None => unreachable!(
                "CRITICAL: WINDOW_SIZE constant overflow in calculate_ms_ssim (WINDOW_SIZE={})",
                WINDOW_SIZE
            ),
        };
        ...
        if let Some(ssim) = calculate_ssim(&orig, &conv) {
            ...
            crate::media_conversion_gate::delivery_runtime_batch_audit(
                "delivery_runtime",
                format!("MS-SSIM scale {i} calculation failed (dim: {w}x{h}); metric invalidated"),
            );
...
mod tests {
    ...
    fn test_identical_images() {
        let img1 = DynamicImage::ImageRgb8(RgbImage::from_fn(100, 100, |x, y| {
            image::Rgb([
                crate::numeric_cast::u32_to_u8_strict(x % 256, "pixel_x").unwrap_or_else(|| {
                    unreachable!(
                        "CRITICAL: x % 256 overflow in test_identical_images (x={})",
                        x
                    )
                }),
                crate::numeric_cast::u32_to_u8_strict(y % 256, "pixel_y").unwrap_or_else(|| {
                    unreachable!(
                        "CRITICAL: y % 256 overflow in test_identical_images (y={})",
                        y
                    )
                }),
                128,
        ...
        assert!(
            psnr.unwrap_or_else(|| {
                unreachable!("CRITICAL: missing metric value (psnr) in test_identical_images")
            })
            .is_infinite()
        );
        ...
        assert!(
            (ssim.unwrap_or_else(|| {
                unreachable!("CRITICAL: missing metric value (ssim) in test_identical_images")
            }) - 1.0)
                .abs()
                < 0.01_f64
        );
    ...
    fn test_different_images() {
        ...
        assert!(
            psnr.unwrap_or_else(|| {
                unreachable!("CRITICAL: missing metric value (psnr) in test_different_images")
            }) < 10.0_f64
        );
        ...
        assert!(
            ssim.unwrap_or_else(|| {
                unreachable!("CRITICAL: missing metric value (ssim) in test_different_images")
            }) < 0.1_f64
        );
    }
    ...
    fn test_ssim_constant_image_equals_one() {
        ...
        assert!(
            (ssim.unwrap_or_else(|| {
                unreachable!(
                    "CRITICAL: missing metric value (ssim) in test_ssim_constant_image_equals_one"
                )
            }) - 1.0)
                .abs()
                < 1e-6_f64
    ...
    fn test_ms_ssim_identical() {
        let img = DynamicImage::ImageRgb8(RgbImage::from_fn(64, 64, |x, y| {
            image::Rgb([
                crate::numeric_cast::u32_to_u8_strict(x.wrapping_add(y) % 256, "pixel_sum")
                    .unwrap_or_else(|| {
                        unreachable!(
                            "CRITICAL: sum % 256 overflow in test_ms_ssim_identical (x={}, y={})",
                            x, y
                        )
                    }),
                128,
        ...
        assert!(
            result.unwrap_or_else(|| {
                unreachable!("CRITICAL: missing metric value (ms_ssim) in test_ms_ssim_identical")
            }) >= 0.99_f64
                && result.unwrap_or_else(|| {
                    unreachable!(
                        "CRITICAL: missing metric value (ms_ssim) in test_ms_ssim_identical"
                    )
                }) <= 1.01_f64
        );
    }
