        ...
        match crate::types::Ssim::new(self.ssim) {
            ...
            Err(e) => {
                crate::media_conversion_gate::probe_quality_batch_audit(
                    "ssim_mapping",
                    format!("invalid mapping SSIM {}: {e}; returning None", self.ssim),
                );
                None
            }
        }
...
impl PsnrSsim {
    ...
    pub fn insert(&mut self, psnr: f64, ssim: f64) {
        ...
        let pos = match self.points.iter().position(|p| p.psnr > psnr) {
            Some(idx) => idx,
    ...
    pub fn predict_ssim_typed(&self, psnr: f64) -> Option<crate::types::Ssim> {
        self.predict_ssim(psnr)
            .and_then(|value| match crate::types::Ssim::new(value) {
                Ok(ssim) => Some(ssim),
                Err(e) => {
                    crate::media_conversion_gate::probe_quality_batch_audit(
                        "ssim_mapping",
                        format!("predicted SSIM {value} is invalid: {e}; returning None"),
                    );
                    None
                }
            })
