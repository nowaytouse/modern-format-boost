...
impl VerifyOptions {
    pub const fn strict_video() -> Self {
        Self {
            ...
            duration_slack_secs: crate::constants::VERIFY_DURATION_TOLERANCE_STRICT,
...
pub fn verify_after_encode(
    ...
    let result = EnhancedVerifyResult {
        file_ok,
        duration_match,
        has_video_stream,
        message,
    ...
    if !result.passed() {
        tracing::warn!(
            target: "mfb.conversion",
            input = %input.display(),
            output = %output.display(),
            message = %result.message,
            "enhanced post-encode verification failed"
        );
    }
    result
}
    ...
    match (input_probe, output_probe) {
        (Ok(ref inp), Ok(ref out)) => {
            ...
            if options.require_duration_match {
                let tol = options.duration_slack_secs.max(0.0);
                let (diff, ok) = match (inp.duration, out.duration) {
                    ...
                    (None, None) => {
                        // Both probes succeeded but neither reported a duration: the check is
                        // unverifiable, not verified. Pass it (nothing contradicts the encode)
                        // but surface the degraded mode to the caller and the log.
                        tracing::warn!(
                            target: "mfb.conversion",
                            input = %input.display(),
                            output = %output.display(),
                ...
                *duration_match = if ok {
                ...
                let diff_str = crate::media_conversion_gate::ui_duration_secs_label_or_na(
                    diff,
                    "quality_verifier_duration_diff",
                );
                details.push(format!(
                    "Duration: input {:?}, output {:?}, diff {} (allowed drift {:.2}s) → {}",
                    inp.duration,
                    out.duration,
                    diff_str,
                    tol,
...
mod tests {
    ...
    #[test]
    fn test_verify_output_file_empty_or_small() -> anyhow::Result<()> {
