//! Strict gates for **media conversion** processing (`img` / `vid` delivery layer) and
//! **probe/detection** paths that feed delivery (`video_detection`, `video_explorer`, `image_analyzer`).
//!
//! Training tier rules live in [`crate::training_tier_audit`]; algorithm seals live in [`crate::algorithm_seal`].
//! All audits emit `[delivery fallback:branch]` for unified log search.
...
/// Explicit audit for delivery-layer fallbacks (never silent). Sole conversion `log_anomaly` site (M100/M101).
...
/// Delivery path/layout fallback (stem/parent/collision): audit only under strict (M87/M97).
...
/// Quality CRF heuristic missing optional field: audit only under strict (M85; probe layer is SSOT).
pub fn quality_heuristic_fallback_audit(branch: &'static str, detail: impl AsRef<str>) {
    probe_quality_batch_audit(branch, detail);
}

/// Missing `content_type` in CRF quality analysis (Unknown adjustment + audit, M104).
...
/// Content type for CRF analysis when analysis omitted it (Unknown + audit, M104/M182).
#[must_use]
...
/// JPEG quantization table cell after IJG scaling (1 + audit when u16 cast fails).
#[must_use]
pub fn delivery_jpeg_qt_cell_u16_or_one(scaled_value: f64, quality: u8) -> u16 {
    crate::numeric_cast::f64_to_u16_strict(scaled_value, "generate_standard_qt").unwrap_or_else(|| {
        delivery_numeric_fallback_audit(
            "delivery_jpeg_qt",
            format!(
                "JPEG QT cell out of u16 range after IJG scaling (quality={quality}, raw={scaled_value}); using floor 1"
            ),
        );
        1
    })
...
/// Probe/detection recovery note (e.g. animated promote): strict via probe layer (M88/M97).
pub fn probe_detection_recovery_audit(branch: &'static str, detail: impl AsRef<str>) {
    probe_layer_batch_audit(branch, detail);
}

/// Single strict-gated path audit entry (M96 SSOT). Domain wrappers delegate here.
///
/// With strict delivery disabled the audit is downgraded to `tracing::debug!`, never
/// dropped: the fallback substitution still happens, so a trace of it must survive.
...
/// Single strict-gated batch audit entry (M96/M98 SSOT). Domain wrappers and gate `*_or_default` helpers delegate here.
///
/// With strict delivery disabled the audit is downgraded to `tracing::debug!`, never
/// dropped: the fallback substitution still happens, so a trace of it must survive.
...
/// `img` / `vid` API path-scoped encode/reconcile fallback (delegates to [`delivery_strict_path_audit`]).
...
/// `img` / `vid` API batch fallback without a stable path (delegates to [`delivery_strict_batch_audit`]).
...
/// Substrate batch audit (delegates to [`delivery_strict_batch_audit`], M95/M96).
fn delivery_substrate_batch_audit(branch: &'static str, detail: impl AsRef<str>) {
    delivery_strict_batch_audit(branch, detail);
}

/// Explore size-target failure label when `failure_reason` is empty (strict audit).
#[must_use]
...
/// Canonical path key for duplicate/processed tracking; logs when canonicalization is unavailable.
///
/// For paths that don't exist yet (e.g., output paths being reserved), canonicalization
/// will fail - this is expected behavior and we silently fall back to display path.
/// For existing files, canonicalization provides a stable key across symlinks and relative paths.
#[must_use]
pub fn processed_path_key(path: &Path) -> String {
    match path.canonicalize() {
        ...
        Err(err) => {
            // Non-existent outputs are expected during reservation; audit only when the path exists.
            if path.exists() {
                delivery_strict_batch_audit(
                    "path_canonicalize",
                    format!(
...
/// Canonical path for external tools; logs and returns non-canonical input when needed.
#[must_use]
...
/// Checkpoint progress-dir root; same resolution policy as [`canonicalize_for_tool_input`] (M120).
#[must_use]
...
/// Optional scalar for audit/format strings only (`0` label means missing; not a measured value).
#[must_use]
...
/// Probe / detection layer audit (same telemetry as delivery; pre-conversion routing, M93/M98).
...
/// DB usize cast for percentile/index derivation (None + DB audit when missing).
#[must_use]
...
/// Image format analysis when no stable path is in scope (in-memory buffer parse).
...
/// GPU detection / encode helper fallback (path-scoped; not explore coarse search).
...
/// Legacy wall-clock duration helper (panics when clock invalid; prefer [`unix_duration_since_epoch_optional`]).
#[must_use]
pub fn unix_duration_since_epoch_or_zero() -> Duration {
    unix_duration_since_epoch_optional().unwrap_or_else(|| {
        panic!(
            "unix_duration_since_epoch required for wall-clock SSOT; clock before UNIX_EPOCH or unavailable"
        );
    })
}

/// Seconds since UNIX epoch for checkpoint/resume timestamps (`None` when clock invalid).
#[must_use]
pub fn unix_epoch_secs_optional() -> Option<u64> {
    unix_duration_since_epoch_optional().map(|d| d.as_secs())
}

/// Legacy wall-clock seconds helper (panics when clock invalid; prefer [`unix_epoch_secs_optional`]).
#[must_use]
...
/// GPU encode concurrency: explicit env wins; otherwise tier cap from `performance_schedule`.
#[must_use]
...
/// Output extension segment for GPU temp filenames (policy-silent `mp4` when missing, M95).
#[must_use]
...
/// Stream-size duration missing/unparseable: audit only under strict delivery (M76).
...
/// Stream-size ffprobe/metadata failures: audit only under strict delivery (M78).
...
/// External tool path for builders, or bare name for PATH lookup (strict-gated).
#[must_use]
pub fn delivery_tool_path_or_bare_name(name: &str) -> PathBuf {
    crate::common_utils::resolve_tool_path(name).unwrap_or_else(|| {
        delivery_strict_batch_audit(
            ...
            format!(
                "external tool '{name}' not resolved (which/user-local/homebrew/cargo fallbacks); using bare name for PATH lookup"
            ),
        );
        PathBuf::from(name)
    })
...
pub fn delivery_path_env_or_empty() -> std::ffi::OsString {
    std::env::var_os("PATH").unwrap_or_else(|| {
        delivery_runtime_batch_audit(
            "process_lock_path",
            "PROCESS LOCK AUDIT: PATH environment variable missing; using empty PATH for tool discovery",
        );
...
pub fn delivery_system_memory_mb_or_zero() -> Option<(u64, u64)> {
    ...
    if res.is_none() {
        delivery_runtime_batch_audit(
            "delivery_system",
            "SYSTEM AUDIT: RAM detection failed during x265 profiling | Forensic: Memory probe returned None; falling back to conservative LowMemory profile to prevent OOM termination",
        );
    }
    res
}
...
pub fn delivery_runtime_permille_u32_or_max(old: u64, new: u64) -> u32 {
    ...
    u32::try_from(permille_u128).unwrap_or_else(|_| {
        delivery_runtime_batch_audit(
            "delivery_numeric",
            format!(
                "RATIO AUDIT: Size change permille overflows u32 | Forensic: old_bytes={old}, new_bytes={new}, permille_raw={permille_u128}; capping display at u32::MAX"
            ),
        );
        u32::MAX
    })
...
/// ffprobe stream `nb_frames` sort priority (`u64::MAX` when absent — not a measured frame count).
#[must_use]
...
pub fn delivery_checkpoint_lock_start_time_or_now(start_time: Option<u64>, now: u64) -> u64 {
    start_time.unwrap_or_else(|| {
        delivery_checkpoint_batch_audit(
            "checkpoint_lock",
            "CHECKPOINT AUDIT: process start time unavailable; using current time for lock metadata",
        );
        now
    })
}
...
/// Measured bit depth for geometry heuristics (`None` when container omits bit depth).
#[must_use]
...
/// Legacy symbol (panics when rate absent; prefer [`probe_r_frame_rate_optional`]).
#[must_use]
...
/// Parsed FPS when rational rate string is invalid (`None` = refuse to forge 0.0 FPS).
#[must_use]
...
/// Parsed FPS when rational rate string is invalid (legacy; prefer `probe_fps_parse_optional`).
...
/// GIF frame count for explore stream analysis (`None` when parse fails or zero frames).
#[must_use]
...
/// Legacy GIF frame count helper (panics when absent; prefer `explore_gif_frame_count_optional`).
#[must_use]
...
/// WebP frame count for explore stream analysis (`None` when parse fails or zero frames).
#[must_use]
...
/// Legacy WebP frame count helper (panics when absent; prefer `explore_webp_frame_count_optional`).
#[must_use]
...
/// Whether a path may be animated when byte sniff fails (falls back to cached format label).
#[must_use]
    ...
    match crate::image_detection::detect_format_from_bytes(path) {
        ...
        Err(err) => {
            probe_image_format_audit(
                ...
                path,
                format!(
                    "Animation-capable check: format sniff failed; using cached label {format_label}: {err}"
                ),
            );
...
pub fn delivery_cli_output_size_label_or_unknown(
    ...
    if let Some(size) = output_size {
        ...
        delivery_pipeline_batch_audit(
            "delivery_pipeline_cli",
            format!(
                "Output path is present but output size is missing (input={input_path}, output={output_path})"
            ),
        );
        String::from("unknown")
    }
...
/// Probe-layer file name for penetration/UI logs (`?` when path has no `file_name`).
#[must_use]
pub fn probe_path_file_name_for_log(path: &Path) -> String {
    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
        ...
        probe_layer_audit(
            ...
            path,
            format!(
                "FIELD AUDIT: Failed to get file name for path '{}' | Forensic: Path is malformed or root; information invalidated",
                path.display()
            ),
        );
        String::from("?")
...
/// GPU coarse-search anchor CRF from warm-start hint (audited baseline when hint missing).
#[must_use]
...
/// Legacy path mtime helper (panics when absent; prefer [`delivery_path_modified_unix_secs_optional`]).
#[must_use]
...
/// Legacy batch depth helper (panics when absent; prefer [`delivery_batch_relative_depth_optional`]).
#[must_use]
...
pub fn delivery_video_sort_work_or_none(pixels: u64, frames: u64, path: &Path) -> Option<u64> {
    if let Some(work) = pixels.checked_mul(frames.max(1)) {
        ...
        delivery_pipeline_path_audit(
            ...
            path,
            format!(
                "NUMERIC ANOMALY: Estimated video sort work overflowed for '{}' (pixels={}, frames={}) | Forensic: u64 overflow during complexity estimation; work metric unavailable for sorting",
                path.display(),
                pixels,
                frames
            ),
...
/// `duration_p90`: empirical collection percentile only (M201/M218; no `feature_stats` substitution).
#[must_use]
...
/// Audited rejection of `feature_stats` collection `duration_p90` (never substitutes empirical P90) (M218).
#[must_use]
pub fn collection_duration_p90_feature_stats(
    ...
    if let Some(value) = feature_stats_p90.filter(|v| v.is_finite()) {
        delivery_db_batch_audit(
            ...
            format!(
                "{context}: ignoring feature_stats duration_p90={value}; empirical sample P90 required"
            ),
        );
    }
    None
...
pub fn loop_profile_feature_absent(feature_key: &str) {
    delivery_db_batch_audit(
        ...
        format!(
            "loop reference feature `{feature_key}` missing from feature_stats; refusing fabricated slot"
        ),
    );
}
...
/// KNN neighbor count for `inference_log` INSERT (`None` when cast overflows) (M201).
#[must_use]
pub fn delivery_db_knn_neighbor_count_i32(count: Option<usize>) -> Option<i32> {
    ...
    if let Some(v) = crate::numeric_cast::usize_to_i32_strict(s, "db_knn_count") {
        ...
        delivery_db_batch_audit(
            "delivery_db_knn",
            "NUMERIC ANOMALY: KNN neighbor count overflow during inference logging | Forensic: usize to i32 conversion failure; refusing to forge telemetry data",
        );
        None
    }
}

/// Skip/failure conversion result output path label (copied dest or input) (M202).
#[must_use]
...
/// `ImageMagick` `identify`: magick binary first, then system `identify` (M202).
...
/// ffprobe stream bit-depth string fields (`bits_per_raw_sample` then `bits_per_sample`) (M203).
#[must_use]
...
/// Encoder settings from ffprobe stream tags (`x265/x264/encoder_settings`) (M203).
#[must_use]
...
/// Mastering-display chromaticity: rational string then scaled f64 fallback (M204).
#[must_use]
...
/// Mastering-display luminance: rational string then scaled f64 fallback (M204).
#[must_use]
...
/// Stream bit depth from `bits_per_raw_sample` then `bits_per_sample` strings (M204).
#[must_use]
...
/// ffprobe avg/`r_frame_rate` with optional native-detected fps fallback (M205).
#[must_use]
...
/// Bitstream media info with WebP canvas fallback when extension is webp (M206).
...
/// width×height pixel count with audited overflow (`None` on u64 overflow) (M202).
#[must_use]
...
/// HEIC deep-analysis canvas when `detect_image` succeeds (`None` = no canvas, not fake 0×0).
#[must_use]
pub fn probe_detection_canvas_optional(
    ...
    detection.map(|d| (d.width, d.height, d.has_alpha, d.bit_depth)).or_else(|| {
        probe_layer_audit(
            "heic_fallback_dimensions",
            path,
            "HEIC fallback: detect_image returned no canvas; refusing fabricated 0×0 dimensions",
        );
        None
    })
}

/// Legacy canvas helper (panics when absent; prefer `probe_detection_canvas_optional`).
#[must_use]
...
/// Fixed-width binary slice read with IO audit when out of range (`None` = refuse to forge).
#[must_use]
...
/// Training CLI `PostgreSQL` DSN: CLI → `MFB_PG_CONNSTR` → audited built-in default.
#[must_use]
...
pub fn img_static_lossless_quality_arg_or_default(
    ...
    if let Some(q) = estimated_quality {
        ...
        (
            String::new(),
            String::from(
                "Static lossy image (non-JPEG), quality estimation failed; recommend AVIF (using encoder defaults)",
            ),
        )
    }
}
...
/// Legacy jxlinfo dimensions helper (returns `None`; prefer [`probe_jxlinfo_dimensions_optional`]).
#[must_use]
...
/// Encoder settings search string for precision metadata (comment tag fallback).
#[must_use]
...
pub fn probe_idet_count_or_zero(path: &Path, field: &'static str, token: Option<&str>) -> u64 {
    probe_idet_count_optional(path, field, token).unwrap_or_else(|| {
        panic!(
            "probe_idet_count_or_zero is legacy — use probe_idet_count_optional ({}, field={field})",
            path.display()
        );
    })
}

/// Exploration CRF when bidirectional fine-tune succeeds (`Err` if refinement absent).
pub fn explore_boundary_crf_optional(
    ...
    refined.ok_or_else(|| {
        explore_precheck_batch_audit(
            ...
            format!(
                "{}: bidirectional fine-tune returned no refinement; refusing silent boundary substitution (boundary_crf={boundary_crf:.2})",
                input.display()
            ),
        );
...
/// Legacy symbol (panics on absent refinement; prefer [`explore_boundary_crf_optional`]).
#[must_use]
...
/// Path-scoped delivery audit emitter (always logs). Call only from [`delivery_strict_path_audit`] (M99/M100).
...
/// Batch delivery audit emitter (always logs). Call only from [`delivery_strict_batch_audit`] (M99/M100).
...
/// Explore / quality-gate rejection on the delivery layer (strict-gated, M89/M96).
...
/// GPU coarse / boundary explore failure with input path (strict-gated, M89/M96).
...
/// WebP frame-duration pad when webpmux frame count disagrees with parsed delays.
pub fn webp_frame_duration_pad_audit(
    ...
    delivery_api_path_fallback_audit(
        ...
        path,
        format!(
            "webpmux reports {frame_count} frames but {parsed_durations} durations parsed; padding tail with {pad_ms}ms"
        ),
    );
}

/// Animated WebP RIFF header frame count when structurally readable (`None` when parse fails or zero).
#[must_use]
...
/// Animated WebP minimum frame policy when animation is confirmed but header count is unreliable.
#[must_use]
    ...
    match probe_webp_header_frame_count_optional(parsed, path) {
        ...
        _ => {
            probe_layer_audit(
                ...
                path,
                "animated WebP without reliable header count; applying minimum frame_count=2 policy",
            );
            2
        }
    }
...
/// Legacy WebP header frame count (`0` when absent; prefer [`probe_webp_header_frame_count_optional`]).
#[must_use]
...
/// Exploration CRF snapped to cache grid; invalid inputs → `NaN` (never fake `0.0` anchor).
#[must_use]
...
/// Host CPU count when `available_parallelism` is unavailable (4 + audit in strict delivery).
#[must_use]
pub fn runtime_available_parallelism_or_default(context: &str) -> usize {
    std::thread::available_parallelism().map_or_else(|err| {
            delivery_strict_batch_audit(
                "runtime_cpu_count",
                format!(
                    "{context}: available_parallelism failed ({err}); using {RUNTIME_CPU_PARALLELISM_DEFAULT}"
                ),
            );
...
/// Animated-container promotion frame count when probe reports ≤1 frame (minimum 2 + audit).
#[must_use]
pub fn probe_animated_promoted_frame_count_or_min_two(frames: Option<u32>, _path: &Path) -> u32 {
    // Minimum 2 frames is intentional vid routing policy, not a forged measurement (no audit).
...
/// Training/delivery feature enabled unless env is explicitly opted out (`0`/`false`/`no`/`off`).
#[must_use]
...
/// Output file size from metadata, or pre-measured encode estimate when metadata is unreadable.
#[must_use]
...
/// Last-resort probe JSON when both payload and error envelope fail to CString-encode (Z8).
...
/// C API probe JSON response pointer (error JSON when serialization fails — never silent `null`).
#[must_use]
...
/// C API ingest path list: JSON `Vec<String>` when valid; pipe-delimited fallback (strict-gated).
#[must_use]
    ...
    match serde_json::from_str::<Vec<String>>(paths_str) {
        ...
        Err(err) => {
            delivery_strict_batch_audit(
                ...
                format!(
                    "ingest paths payload is not a JSON string array ({err}); using pipe-delimited parse"
                ),
            );
            split_ffi_ingest_paths(paths_str)
        }
...
/// JXL explore previous screened candidate size (audited baseline on first probe).
#[must_use]
...
/// GIF logical screen dimensions from header prefix (`None` when unreadable or zero).
#[must_use]
...
/// Legacy GIF logical screen helper (`(0, 0)` when absent; prefer [`loop_gif_logical_screen_optional`]).
#[must_use]
...
/// Loop-intent filename segment for numeric-density features (empty when unknown).
#[must_use]
...
/// Clear percentile slots on a distribution that is not backed by an empirical histogram.
...
/// Require an empirical feature-map slot when building a DB-backed loop reference profile.
///
/// # Errors
...
/// Log-line file-type icon prefix from filename extension (empty when no extension).
#[must_use]
...
/// GIF encode FPS from probe (audits when not derived from extracted frames / duration).
#[must_use]
...
/// Inline SSIM segment for progress bars (empty when unset; strict-gated audit, M113).
#[must_use]
...
/// SSIM token for exploration progress bar output (delegates to [`ui_ssim_inline_or_empty`]).
#[must_use]
pub fn explore_progress_ssim_token(ssim: Option<f64>, context: &str) -> String {
    ui_ssim_inline_or_empty(ssim, context)
}

/// SSIM inline segment when SSIM has not been measured yet (no audit; expected during encode).
#[must_use]
...
/// Per-iteration explore progress SSIM (encode in flight; absence is not a delivery fallback).
#[must_use]
pub fn explore_progress_ssim_token_pending(ssim: Option<f64>) -> String {
    ui_ssim_inline_when_unmeasured(ssim)
}

/// Infrastructure label when an optional version string is missing (e.g. pgvector).
#[must_use]
...
/// Resolve [`crate::ffprobe_json::ColorInfo`] for cjxl prep; probes via ffprobe when not supplied.
#[must_use]
...
/// ffprobe stream `pix_fmt` (empty when missing — alpha-aux heuristic; policy-silent, M92).
#[must_use]
...
/// Batch audit for analysis-cache DB lifecycle (CLI init, age prune, algorithm upgrade).
...
/// Video analysis cache store failure (SSOT for `detect_video_with_cache` and `conversion_api`).
...
/// Legacy cache prune helper (`0` when parse fails; prefer [`analysis_cache_prune_rows_optional`]).
...
/// Legacy VP8X flags helper (panics when absent; prefer [`probe_webp_vp8x_flags_optional`]).
#[must_use]
...
/// Video explore precheck / stream / SSIM path-scoped audit (strict-gated, M96).
...
/// Misuse of contract `*_or_default` exports (logic error — production must use `*_optional` / `?`).
#[cold]
#[track_caller]
...
/// Explore metric parse/seal reject: audit only under strict delivery (M75 anti-spam).
...
/// Explore precheck / `stream_analysis` operational failure (delegates to [`explore_precheck_batch_audit`], M84/M96).
pub fn explore_precheck_degraded_audit(branch: &'static str, detail: impl AsRef<str>) {
    explore_precheck_batch_audit(branch, detail);
}

/// SSIM/CAMBI/MS-SSIM operational failure (probe/IO/parse): audit only under strict (M79).
...
/// `nb_frames` absent in precheck JSON (`None`; audit only — not a measured frame count).
#[must_use]
...
/// Legacy sentinel helper (deprecated; prefer [`explore_precheck_nb_frames_optional`]).
#[must_use]
...
/// Resolved `nb_frames` from precheck JSON (`None` when absent; callers re-derive from duration×fps).
#[must_use]
...
/// Explore outcome / safety stop (highly compressed, iteration cap): strict-gated (M84/M96).
...
/// Legacy progress millis helper (panics internally via `unwrap` on overflow path; prefer optional).
#[must_use]
pub fn explore_progress_time_millis_or_zero(time_us: u64) -> u32 {
    explore_progress_time_millis_optional(time_us).unwrap_or_else(|| {
        panic!(
            "explore_progress_time_millis_or_zero is legacy — use explore_progress_time_millis_optional (time_us={time_us})"
        );
    })
}

/// GPU coarse search / phase-3 explore fallback (path-scoped, strict-gated, M96).
pub fn explore_gpu_coarse_audit(branch: &'static str, input: &Path, detail: impl AsRef<str>) {
    delivery_strict_path_audit(branch, input, detail);
}

/// GPU coarse search fallback when no stable input path is in scope (strict-gated, M96).
pub fn explore_gpu_coarse_batch_audit(branch: &'static str, detail: impl AsRef<str>) {
    delivery_strict_batch_audit(branch, detail);
}

/// GPU coarse degrading / explore diagnostic batch audit (strict-gated, M77/M83/M96 SSOT).
...
/// GPU coarse explore-phase diagnostic (alias of [`explore_gpu_coarse_fallback_audit`], M83/M96).
...
/// Audio bitrate for GPU coarse transcode strategy (`None` when probe omits or zero `bit_rate`).
#[must_use]
...
/// Legacy name (panics when absent; prefer [`explore_gpu_coarse_audio_bitrate_optional`]).
#[must_use]
...
pub fn process_exit_code_for_context(
    ...
    code.unwrap_or_else(|| {
        delivery_strict_batch_audit(
            "process_exit_code",
            format!("{tool} ({context}): exit code unavailable (subprocess reap/killing failure); reporting -1"),
        );
        -1
    })
}
...
/// Serialize stderr/progress terminal output with audited poison recovery (M49/M169).
...
/// Recover a poisoned `RwLock` read guard (tool cache / builder infrastructure).
...
/// Recover a poisoned `RwLock` write guard (tool cache / builder infrastructure).
...
pub fn delivery_batch_output_bytes_or_input(
    ...
    output_bytes.unwrap_or_else(|| {
        delivery_strict_batch_audit(
            ...
            format!(
                "{context}: output_size missing after reported success; using input_size {input_bytes} for batch totals"
            ),
        );
        input_bytes
    })
...
/// Human-readable size delta for conversion reports (`i64` overflow → audited label).
#[must_use]
...
/// Exploration CRF anchor when warm-start cache is empty (audited; not a measured encode result).
#[must_use]
pub fn warm_start_crf_or_predicted(
    ...
    // Fail-closed: when warm-start cache is empty, do not drive exploration from the
    // derived/predicted anchor. Use codec defaults as an audited conservative baseline.
    ...
    explore_gpu_coarse_fallback_audit(
        ...
        format!(
            "{}: no warm-start CRF for {codec}; using codec default search anchor {codec_default:.2} (predicted {predicted:.2} ignored)",
            input.display()
        ),
    );
    codec_default
...
/// SSIM fragment for conversion success messages (empty when absent; strict-gated audit, M113).
#[must_use]
...
/// `FFmpeg` / process exit-code suffix for user-facing errors (empty when unknown + audit, M113).
#[must_use]
...
/// Optional metadata extension (empty when missing; audit only in strict delivery).
#[must_use]
...
/// Optional metadata container label (empty when missing; audit only in strict delivery).
#[must_use]
...
/// Parent directory for metadata/XMP sidecar resolution (audited `.` when at filesystem root).
#[must_use]
...
/// UTF-8 file stem required for strict UTF-8 consumers (errors instead of silent fallback).
///
/// # Errors
...
/// File stem for output path joins (`OsStr`; errors only when stem segment is missing).
///
/// # Errors
/// Returns a delivery error message when stem is missing (non-UTF-8 stems remain joinable).
...
/// File stem for metadata/XMP routing (lossy when non-UTF-8; empty + audit only when missing).
#[must_use]
...
/// File name for XMP/metadata matching (empty when missing; audit only in strict delivery).
#[must_use]
...
/// Primary decode error when secondary bitstream recovery also fails (audited generic fallback).
#[must_use]
...
/// Legacy rational helper (`Rational(0)` when absent; prefer [`probe_rational_from_f64_optional`]).
#[must_use]
...
/// Weighted JPEG quality after strict u8 cast (audited fallback to luma estimate).
#[must_use]
...
/// End index of a leading numeric metric token in ffmpeg/libvmaf stderr (not a delivery fallback).
#[must_use]
...
/// Buffer prefix slice for HDR/ICC scans (audited empty when `end` exceeds length).
#[must_use]
...
/// x265 params text segment when merging encoder flags (`None` → empty, no audit).
...
/// Explore GPU coarse-search CRF anchor when `best_crf` is unset during backtrack.
#[must_use]
pub fn explore_best_crf_or_backtrack_anchor(
    ...
    best_crf.unwrap_or_else(|| {
        explore_gpu_coarse_fallback_audit(
            "best_crf",
            format!(
                "GPU explore backtrack: best_crf missing; using test_crf+step ({test_crf:.2}+{old_step:.2})"
            ),
        );
        test_crf + old_step
    })
...
/// Temp/backup file extension label (audited `tmp` when output has no extension).
#[must_use]
...
/// First whitespace-delimited token from ffmpeg/statistics stdout (audited when empty).
#[must_use]
...
/// Still-image `FFmpeg` pipe RGB `pix_fmt` routed through [`crate::media_precision::ImagePrecisionProfile`] (M67).
#[must_use]
...
/// PNG16 preservation decode RGB `pix_fmt` name (excludes float to misleading 48-bit PNG pipe) (M67).
#[must_use]
...
/// Animated GIF/WebP color richness in `[0, 1]` from palette size or entropy fallback (M67).
#[must_use]
...
/// Optional finite probe feature: missing/non-finite → `NaN` (audit only; not a measured value).
...
/// Optional quality-embedding scalar: missing/non-finite → `NaN` (never `0.0` as fake measurement).
///
/// Gate audit helpers do **not** legitimize fabrication; callers must not treat `NaN` as measured.
...
/// Unit-interval embed slot from optional scalar (`NaN` when absent; no probe audit).
#[must_use]
...
/// CPU calibration encode `pix_fmt` when ffprobe probe is present (`None` if absent).
#[must_use]
...
/// Legacy symbol (panics when probe absent; prefer [`explore_calibration_pix_fmt_optional`]).
#[must_use]
...
/// Measured ffprobe duration for calibration (`None` if absent; never substitutes sample window).
#[must_use]
...
/// Legacy name (panics when duration absent; prefer [`explore_calibration_duration_optional`]).
#[must_use]
...
/// Dynamic GPU→CPU calibration failure (probe/encode/read): audit only under strict (M81).
...
/// Legacy calibration size helper (panics when absent; prefer [`explore_calibration_probe_size_optional`]).
#[must_use]
...
/// GPU coarse-search sample window from measured ffprobe duration (`None` if absent).
#[must_use]
...
/// Legacy symbol (panics when absent; prefer [`explore_gpu_sample_duration_optional`]).
#[must_use]
...
/// Legacy symbol (panics when baseline absent; prefer [`explore_adaptive_vmaf_y_floor_optional`]).
#[must_use]
...
/// Legacy symbol (panics when baseline absent; prefer [`explore_adaptive_psnr_uv_floor_optional`]).
#[must_use]
...
/// Parse positive finite ffprobe duration text; return `None` on failure (caller audits if needed).
#[must_use]
...
/// Chroma subsampling factor for CRF heuristics when `pix_fmt` is known (`None` if absent).
#[must_use]
...
/// Legacy name (panics when `pix_fmt` absent; prefer [`probe_chroma_factor_optional`]).
#[must_use]
...
/// Compression ratio for GPU quality scoring when `input_size` may be zero (refuses fabricated 1.0).
#[must_use]
pub fn gpu_quality_compression_ratio_or_neutral(
    ...
    if input_size == 0 {
        delivery_gpu_batch_audit(
            ...
            format!(
                "{context}: input_size is 0 during quality score calculation; refusing fabricated compression ratio 1.0"
            ),
        );
        return f64::NAN;
    }
...
/// GPU coarse boundary CRF: quality ceiling when detected, else search `best_crf` when boundary found.
#[must_use]
...
/// Legacy symbol (panics when boundary absent; prefer [`explore_gpu_boundary_crf_from_search_optional`]).
#[must_use]
...
/// Phase-1A GPU search size when `best_size` must exist (logic-error path; never fabricates `0`).
#[must_use]
...
/// Legacy Phase-1A size helper (delegates to [`delivery_gpu_phase_best_size_required`]; no silent `0`).
#[must_use]
...
/// Binary-search CRF probe from `mid` (audited u16 fallback via `hi` when `mid` overflows).
#[must_use]
pub fn delivery_gpu_binary_search_crf_from_mid(mid: i32, hi: i32) -> f32 {
    let mid_u16 = u16::try_from(mid.max(0_i32)).unwrap_or_else(|_| {
        delivery_gpu_batch_audit(
            "delivery_gpu",
            format!(
                "NUMERIC AUDIT: Binary search mid {mid} overflows u16 | Forensic: falling back to hi {hi} or MAX (Search integrity maintained)"
            ),
        );
...
/// Legacy symbol (panics on calibration failure; call `quick_calibrate` and propagate `Err`).
#[must_use]
...
/// Encode size improvement % when a prior best exists (`None` for first sample).
#[must_use]
...
/// Legacy name (panics when no prior best; prefer [`explore_encode_size_improvement_pct_optional`]).
#[must_use]
pub fn explore_encode_size_improvement_pct(
    ...
    explore_encode_size_improvement_pct_optional(best_size, new_size, context).unwrap_or_else(|| {
        gate_legacy_optional_misuse(
            "explore_encode_improvement",
            format!("{context}: no prior best_size; use explore_encode_size_improvement_pct_optional"),
        );
    })
}

/// Ultimate-mode VMAF gate sample rate from measured duration hint (`None` if absent).
#[must_use]
...
/// Legacy symbol (panics when duration hint absent; prefer [`explore_ultimate_gate_sample_rate_optional`]).
#[must_use]
...
/// Legacy latest-size helper (panics when absent; prefer `explore_latest_encoded_size_optional`).
#[must_use]
...
/// Explore audit elapsed seconds when start instant is missing (`None`, not fake `0.0s`).
#[must_use]
...
/// Legacy explore elapsed helper (`NaN` when unset; prefer `explore_elapsed_secs_optional` in UI).
...
/// Dynamic GPU→CPU mapping offset from anchor size ratio (`NaN` when anchor missing).
#[must_use]
...
/// Legacy JXL telemetry helper (panics when absent; prefer `jxl_best_telemetry_optional`).
#[must_use]
...
/// Runtime elapsed seconds when start instant was recorded (`None` when timer never armed).
#[must_use]
...
/// Legacy runtime elapsed helper (`0` when unset; Ctrl-C watcher only — not a quality metric).
#[must_use]
...
/// Legacy GIF GCT size helper (panics when absent; prefer [`gif_palette_byte_size_optional`]).
#[must_use]
pub fn gif_palette_byte_size_or_zero(palette_colors: Option<u32>, context: &str) -> usize {
    gif_palette_byte_size_optional(palette_colors, context).unwrap_or_else(|| {
        panic!(
            "gif_palette_byte_size_or_zero is legacy — use gif_palette_byte_size_optional ({context})"
        );
    })
}
...
/// Legacy GPU compression-potential helper (panics when absent; prefer optional + skip addend).
#[must_use]
...
/// Explore progress CRF (unset until first probe; poison → `None`, never fake `0.0`).
#[must_use]
...
/// Explore progress best SSIM (unset until first measurement; poison → `None`, never fake `0.0`).
#[must_use]
...
/// Explore-progress optional metric when mutex is poisoned (treat as unset; M46).
#[must_use]
...
///
/// For global operations (e.g., database connections) without a specific file context,
/// returns the reason directly without warning - this is expected behavior.
/// For file-specific operations, includes the path in the output.
#[must_use]
...
/// Inline SSIM label for progress bars (`N/A` when unset; audited in strict delivery).
#[must_use]
...
/// Finite f64 for status lines or a placeholder (audited in strict delivery; M46).
#[must_use]
...
/// Optional f64 with custom finite formatting or a placeholder (audited in strict delivery).
#[must_use]
...
/// Duration label for logs (`12.3s`) or audited `Unknown` when unset (U12 / DB layer).
#[must_use]
...
pub fn probe_imagemagick_animation_detected_audit(
    ...
    probe_layer_audit(
        ...
        path,
        format!(
            "[Duration Fallback] ImageMagick animation detected: {frame_count} frames, {duration_secs:.2}s ({})",
            path.display()
        ),
    );
}
...
/// Colored terminal label for [`crate::infra::static_logs::ErrorSeverity`] (M55 / L8).
#[must_use]
...
/// Static not-applicable label for recommendation structs (audited in strict delivery).
#[must_use]
...
/// Internal absent-measurement marker for legacy loop helpers (not a fabricated score).
#[inline]
#[must_use]
...
/// Loop duration z-score neutral when metadata duration is missing (`NaN`, not fake `0.0`).
#[must_use]
...
/// Legacy bytes-per-frame helper (`NaN` when absent; prefer [`loop_bytes_per_frame_optional`]).
#[must_use]
...
/// Scaled duration percentile: `Some(p) => p * scale`; missing primary uses unscaled `fallback_secs` + audit.
#[must_use]
    ...
    match (width, height) {
        ...
        _ => {
            delivery_intent_batch_audit(
                "loop_total_pixels",
                format!(
                    "{context}: missing or invalid width/height; refusing fabricated total_pixels 0.0"
                ),
            );
            None
        }
...
/// Legacy total-pixels helper (`NaN` when absent; prefer [`loop_total_pixels_optional`]).
#[must_use]
...
/// Duration z-score when metadata duration is missing (neutral 0.0 via [`loop_missing_duration_z_neutral`]).
#[must_use]
...
/// Frame-count label for intent audits when metadata count is unknown (`unknown` + audit).
#[must_use]
...
/// Layer tag extracted from a verdict reason (`Unknown` + audit when unparsable).
#[must_use]
...
/// FPS kinetic z-score weights when fps metadata is missing (zero contribution, not measured fps=0).
#[must_use]
...
/// JXL oversize comparison sentinel when screened candidate index is invalid (`u64::MAX` + audit).
#[must_use]
...
/// Animated/static feature compression ratio when analysis omits it (audited heuristic estimate).
#[must_use]
...
/// Animated-image frame delay variation when timing probes fail (audited VFR/CFR default).
#[must_use]
...
/// UTF-8 prefix slice for display truncation (audited empty when `end` exceeds length).
#[must_use]
...
/// UTF-8 suffix slice for stderr/token parsing (audited empty when `start` exceeds length).
#[must_use]
...
/// Loop/training ingest physics vector when embedding slice is absent (strict DB audit).
#[must_use]
...
/// KNN pgvector component when an optional sample feature is absent (L2 sparse origin; **not** a measured metric).
#[must_use]
pub const fn knn_absent_feature_component() -> f64 {
    0.0_f64
}

/// Min/avg/max triple from DB samples; absent corpus → `None` (never forged `(0,0,0)`).
#[must_use]
...
/// Legacy name; returns `NaN` triple when samples absent (callers must use `db_numeric_stats_triple_or_none`).
...
/// Relative path parent for timestamp restore walks (audited when at relative root).
#[must_use]
...
/// Optional explore-progress f64 suffix (empty when unset; not a delivery fallback).
#[must_use]
...
/// Optional explore best-CRF suffix (empty when unset; not a delivery fallback).
#[must_use]
...
/// Safety relative-path base: `current_dir` when available, else audited `/` (conservative lex normalization).
#[must_use]
...
/// Absolute output path for tools that require it (`cwd` join, or audited `.` fallback).
#[must_use]
...
/// Tracing registry filter: honor `RUST_LOG` when valid; audited fallback to config level.
#[must_use]
...
/// OS system temp directory — gate-internal SSOT for `std::env::temp_dir()` (M169).
#[must_use]
...
/// Scratch/temp directory for delivery artifacts (`get_mfb_tmp_dir` → audited system temp).
#[must_use]
...
/// RAII temp directory under MFB scratch (`get_mfb_tmp_dir` → audited system temp).
...
/// Best-effort `create_dir_all` with path audit on failure (no silent `let _ =` discard).
...
/// Legacy fixed staging path helper (audited when destination has no extension).
#[must_use]
...
/// Best-effort file removal with cleanup audit (no silent `let _ = remove_file`).
...
/// Ensure an output path's parent exists; audit root outputs and mkdir failures.
...
/// Named temp file beside a final output path (atomic persist via `persist`); audits missing parent.
...
/// Disk-space probe path when CLI output dir is unset (defaults to input; strict audit).
#[must_use]
...
/// External tool executable when builder override is unset (audited default binary name).
#[must_use]
...
/// First segment before a separator for metadata date strings (whole string when split is empty).
#[must_use]
...
/// Sorted distance sample at index with clamp to nearest endpoint; empty slice → `None`.
#[must_use]
...
/// Parent directory segment count for loop metadata (0 when ancestry is unknown + audit).
#[must_use]
...
/// Optional loop duration percentile with an explicit baseline constant (strict audit).
///
...
/// Collection duration field fallback; skip audit when profile lacks percentile histogram (M122/M221).
#[must_use]
...
/// ffprobe reported 0×0 (or invalid) dimensions; bitstream header supplied canvas size (M122).
pub fn probe_bitstream_dimension_recovery_audit(
    ...
    probe_layer_audit(
        ...
        path,
        format!(
            "ffprobe stream dimensions {probe_width}x{probe_height} invalid; recovered {recovered_width}x{recovered_height} from container bitstream"
        ),
    );
}
...
/// Inference log JSON field for optional finite `f64` (`null` when missing/non-finite).
#[must_use]
...
/// Sparse inference telemetry: optional `f64` → JSON `null` without batch audit (M117).
#[must_use]
...
/// Sparse inference telemetry: optional `i32` → JSON `null` without batch audit (M117).
#[must_use]
...
/// Sparse inference telemetry: optional `bool` → JSON `null` without batch audit (M117).
#[must_use]
pub fn json_inference_optional_bool_or_null(value: Option<bool>) -> serde_json::Value {
    value.map_or(serde_json::Value::Null, serde_json::Value::from)
}

/// Sparse inference telemetry: optional string → JSON `null` without batch audit (M117).
#[must_use]
...
/// Loop duration fallback between two resolved percentile values (strict audit when primary missing).
///
/// Prefer [`loop_duration_or_fallback_policy`] in production so empirical histogram provenance is explicit.
#[must_use]
...
/// Collection `duration_p90` for threshold math: sample-derived P90 only, or audited feature-map fallback (M218/M219/M224).
#[must_use]
pub fn loop_collection_duration_p90_or_baseline(
    ...
    if profile_percentiles_available {
        if let Some(v) = collection_p90.filter(|v| v.is_finite()) {
            delivery_db_batch_audit(
                ...
                format!(
                    "{context}: ignoring non-sample {field}={v} despite profile histogram; using baseline {baseline}"
                ),
            );
        }
        return baseline;
    ...
    if let Some(v) = collection_p90.filter(|v| v.is_finite()) {
        delivery_db_batch_audit(
            "collection_duration_p90_discarded",
            format!(
                "{context}: ignoring {field}={v} from feature_stats without sample or histogram provenance; using baseline {baseline}"
            ),
        );
    }
    baseline
...
/// Median duration for short-clip thresholding; absent without empirical profile histogram (M219).
#[must_use]
...
/// Loop duration fallback; skip audit when reference profile has no percentile histogram (M117).
#[must_use]
...
/// Scaled loop duration percentile; skip audit when profile has no percentile histogram (M117).
#[must_use]
...
/// Algorithm env flag (`1` = enabled); unset uses `default_enabled` without audit.
#[must_use]
...
/// Algorithm env `usize` parse with documented default (strict audit when unset/invalid).
#[must_use]
        ...
        Ok(raw) => match raw.trim().parse::<usize>() {
            ...
            Ok(_) => {
                delivery_strict_batch_audit(
                    "algorithm_env_usize",
                    format!(
                        "{context}: env {env_key} must be greater than zero; using default {default}"
                    ),
                );
                default
            }
            Err(e) => {
                delivery_strict_batch_audit(
                    "algorithm_env_usize",
                    format!(
                        "{context}: failed to parse env {env_key}='{raw}': {e}; using default {default}"
                    ),
                );
                default
            }
...
/// Metadata timestamp restore error when the first failure reason was not captured.
#[must_use]
...
/// KNN feature weight from stats map (`None` when weight absent — not a forged default).
#[must_use]
...
/// Legacy name; panics after audit when weight missing (prefer [`db_feature_weight_optional`]).
#[must_use]
...
/// Lossy path extension string for depth-map outputs (empty when missing; strict audit).
#[must_use]
...
/// `FFprobe` JSON value → `f64` extraction: try string parse first, then native `as_f64` (M204).
///
/// Centralises the two-step parse previously inlined as `.or_else(|| raw.as_f64())`.
#[must_use]
...
/// Last non-empty stderr line for encode/tool failures (audited when stderr is empty).
#[must_use]
...
/// Lowercase extension for bulk scans/filters (missing extension is normal; no audit).
#[must_use]
...
/// When strict media conversion is on, CLI/API size-growth allowance is suppressed.
#[must_use]
...
/// Central explore delivery gate (single source for `ExploreResult::pipeline_acceptable`).
#[must_use]
...
/// Align [`ImageAnalysis::is_animated`] with the same byte/ffprobe logic as [`animation_reject_outcome`].
///
/// Format **family** comes from content sniffing (`analysis.format`), not from trusting the path
/// extension alone. This prevents legacy analyzer heuristics (e.g. GIF duration/GCE hints) from
/// disagreeing with the img static gate on true single-frame assets.
                ...
                match crate::image_detection::animatable_format_confirmed_static_only(
                    ...
                    Err(err) => {
                        probe_layer_audit(
                            ...
                            path,
                            format!(
                                "Failed to confirm static-only animated-capable format; preserving detected animation flag: {err}"
                            ),
                        );
                        false
                    }
...
///
/// Animation ambiguity and [`ImageAnalysis::analysis_error`] always yield [`StaticConversionVerdict::Ignore`].
/// Entropy trust and exploration relaxations honour `strict_media_conversion_delivery_enabled` (see `algorithm_runtime`).
#[must_use]
...
/// Structured img ignore for animation detection (fast preflight + full analysis).
#[derive(Debug, Clone, PartialEq, Eq)]
        ...
        Ok((false, frame_count, _)) => {
            match crate::image_detection::animatable_format_confirmed_static_only(
                ...
                Ok(true) => {
                    if analysis.is_animated {
                        probe_detection_recovery_audit(
                            ...
                            format!(
                                "{}: multifaceted verification confirmed true single-frame/static; img proceeds",
                                input.display()
                            ),
                        );
                    }
                ...
                Ok(false) => Some(ImgIgnoreOutcome {
                    reason: format!(
                        "Cannot confirm static-only for {} (multi-stream/cover ambiguity or latent animation) — {IMG_STATIC_ONLY}",
                        analysis.format
                    ),
...
mod tests {
    ...
    /// Single-frame `GIF89a` with GCE (timing extension only) must not be rejected for GCE alone.
    #[test]
