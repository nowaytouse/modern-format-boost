...
pub fn calculate_ms_ssim_standalone(reference: &Path, distorted: &Path) -> Result<f64> {
    let ref_y4m_file = crate::media_conversion_gate::delivery_named_tempfile_in_scratch_or_err(
        "vmaf_ref_y4m",
        Some("vmaf_ref_"),
        Some(".y4m"),
    )
    .context("Failed to create ref temp file")?;
    let dist_y4m_file = crate::media_conversion_gate::delivery_named_tempfile_in_scratch_or_err(
        "vmaf_dist_y4m",
        Some("vmaf_dist_"),
        Some(".y4m"),
    )
    .context("Failed to create dist temp file")?;
    let json_file = crate::media_conversion_gate::delivery_named_tempfile_in_scratch_or_err(
        "vmaf_result_json",
        Some("vmaf_result_"),
        Some(".json"),
    )
...
fn parse_vmaf_json(path: &Path) -> Result<f64> {
    ...
    crate::video_explorer::precision::seal_ms_ssim(ms_ssim)
        .context("MS-SSIM mean out of [0,1] finite domain")
}
...
mod tests {
    ...
    #[test]
    fn test_parse_vmaf_json_rejects_out_of_range() {
        let temp = TempDir::new().unwrap();
        let json_path = temp.path().join("vmaf_output_oob.json");
        ...
        fs::write(&json_path, oob_json).unwrap();
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("out of [0,1] finite domain")
        );
    }
}
