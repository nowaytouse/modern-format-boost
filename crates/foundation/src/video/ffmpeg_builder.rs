...
impl PixFmt {
    ...
    pub const fn ffmpeg_name(&self) -> &'static str {
        match self {
            ...
            Self::Rgb48le => constants::PIX_FMT_RGB48LE,
...
impl FfmpegBuilder {
    ...
    pub fn vcodec_str<S: AsRef<str>>(&mut self, codec: S) -> &mut Self {
        VideoCodec::from_str(codec.as_ref()).map_or_else(
            |()| {
                self.base.arg("-c:v");
    ...
    fn build(&self) -> Command {
        ...
        if let Some(preset) = self.preset {
            ...
            if matches!(self.vcodec, Some(VideoCodec::Av1)) {
                cmd.arg(preset.sanitize_av1().svtav1_preset().to_string());
...
mod tests {
    ...
    fn input_format_is_emitted_before_input_paths() {
        ...
        assert_eq!(
            args.first().unwrap_or_else(|| panic!("missing 0")),
            "-hide_banner"
        );
        ...
        assert_eq!(
            args.get(4).unwrap_or_else(|| panic!("missing 4")),
