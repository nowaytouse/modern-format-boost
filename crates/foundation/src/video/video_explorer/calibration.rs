...
mod tests {
    ...
    fn test_point_from_gpu_result() {
        ...
        assert!(
            (p.predicted_cpu_crf - 25.0).abs() < 1e-6,
            "predicted_cpu_crf mismatch: got {}",
            p.predicted_cpu_crf
        );
        assert!(
            (p.confidence - 0.85).abs() < 1e-6,
            "confidence mismatch: got {}",
            p.confidence
        );
        ...
        assert!(
            (p2.predicted_cpu_crf - 23.0).abs() < 1e-6,
            "predicted_cpu_crf mismatch: got {}",
            p2.predicted_cpu_crf
        );
        assert!(
            (p2.confidence - 0.70).abs() < 1e-6,
            "confidence mismatch: got {}",
            p2.confidence
        );
    }
