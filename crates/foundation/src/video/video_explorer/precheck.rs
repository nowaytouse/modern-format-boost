...
fn parse_stream_bit_depth(stream: &serde_json::Value, pix_fmt: Option<&str>) -> (Option<u8>, bool) {
    ...
    if let Some(inferred) = pix_fmt.and_then(crate::ffprobe::detect_bit_depth) {
...
) -> crate::ffprobe_json::ColorInfoAssessment {
    let hdr = crate::ffprobe::extract_hdr_side_data(stream);
    crate::ffprobe_json::ColorInfoAssessment::from_probe_fields(
        color_space,
        color_transfer,
        color_primaries,
        crate::BitDepthMetadata::new(bit_depth, bit_depth_inferred_from_pix_fmt),
        crate::ffprobe_json::ColorProbeFlags {
            has_mastering_display: hdr.mastering_display.is_some(),
            has_max_cll: hdr.max_cll.is_some(),
            is_dolby_vision: hdr.is_dolby_vision(),
            is_hdr10_plus: hdr.hdr10_plus,
            is_float: crate::ffprobe_json::pix_fmt_indicates_float(pix_fmt),
        },
    )
}
...
fn fps_sanitise_for_validation(fps: f64, duration: f64, frame_count: Option<u64>) -> f64 {
    ...
    {
        let inferred = crate::numeric_cast::u64_to_f64(frames) / duration;
...
fn materialize_frame_count(frame_count: Option<u64>, duration: f64, fps: f64) -> Result<u64> {
    ...
    if duration > 0.0_f64 && fps.is_finite() && fps > 0.0_f64 {
        return crate::numeric_cast::f64_to_u64_strict(duration * fps, "frame_count")
    ...
    bail!("frame count absent and cannot derive from duration×fps")
}

fn parse_duration_from_precheck_json(
    json: &serde_json::Value,
    fps: f64,
    mut frame_count: Option<u64>,
    input: &Path,
) -> Result<(f64, f64, Option<u64>)> {
    let stream = json["streams"].get(0);

    // If frame_count is absent/zero, try nb_read_frames (APNG / -count_frames).
    if frame_count.is_none_or(|n| n == 0)
        && let Some(nb_read_frames) = crate::numeric_cast::parse_option_strict(
            stream.and_then(|s| s["nb_read_frames"].as_str()),
            "nb_read_frames",
        )
    {
        frame_count = Some(nb_read_frames);
        crate::log_detail!(format!(
            "Using nb_read_frames ({nb_read_frames}) for frame count"
        ));
    }
...
fn bpp_from_precheck_json(json: &serde_json::Value, file_size: u64, input: &Path) -> Result<f64> {
    ...
    let nb_frames_option = match nb_frames_parsed {
        Some(v) => Some(v),
        None => nb_frames_u64,
    };
    let frame_count_raw = crate::media_conversion_gate::explore_precheck_nb_frames_resolved(
        nb_frames_option,
        input,
        "nb_frames absent or non-numeric (will re-derive from duration)",
    );
    ...
    let frame_count = materialize_frame_count(frame_count_raw, duration, fps)?;
    let video_bytes =
...
pub fn detect_duration_comprehensive(input: &Path) -> Result<(f64, f64, u64, &'static str)> {
    ...
    let frame_count_opt = crate::media_conversion_gate::explore_precheck_nb_frames_resolved(
        json.get("streams")
            .and_then(|s| s.get(0))
        ...
        input,
        "nb_frames absent or non-numeric",
    );
    ...
    if let Some(duration) = stream_duration {
        let fps = fps_sanitise_for_validation(fps, duration, frame_count_opt);
        let frame_count = materialize_frame_count(frame_count_opt, duration, fps)?;
    ...
    if let Some(frames) = frame_count_opt.filter(|&c| c > 0)
        && fps > 0.0_f64
        && !fps.is_nan()
        && fps <= FPS_THRESHOLD_INVALID
    {
...
pub fn get_video_info(input: &Path) -> Result<VideoInfo> {
    ...
    let codec = crate::media_conversion_gate::probe_ffprobe_codec_name_lowercase(
    ...
    let bitrate_kbps = match crate::numeric_cast::parse_option_strict::<f64>(
        stream["bit_rate"].as_str(),
        "bit_rate",
    ) {
        None => {
            if duration > 0.0_f64 {
    ...
    let (bit_depth, bit_depth_inferred_from_pix_fmt) =
        parse_stream_bit_depth(stream, pix_fmt.as_deref());

    let fps_category = FpsCategory::from_fps(fps);
    let color_assessment = assess_stream_color_info(
        stream,
...
fn evaluate_processing_recommendation(
    ...
    let Some(base_bitrate_r) =
        ...
        crate::media_conversion_gate::probe_layer_batch_audit(
            "explore_precheck",
            "NUMERIC ANOMALY: internal base_bitrate constant is non-finite; cannot score bitrate",
        );
        return ProcessingRecommendation::CannotProcess {
            reason: "Internal precheck bitrate baseline invalid (NaN/Inf)".to_string(),
        };
    };
...
pub fn print_precheck_report(info: &VideoInfo) {
    ...
    if info.color_space.is_some() || info.pix_fmt.is_some() || info.bit_depth.is_some() {
        ...
        if let Some(bd) = info.bit_depth {
            let provenance = if info.bit_depth_inferred_from_pix_fmt {
                " (pix_fmt-inferred)"
            } else {
                ""
            };
            lines.push(format!("│ Bit Depth: {bd}-bit{provenance}"));
        }
    }
    ...
    lines.push("└─────────────────────────────────────────────────────".to_string());
    for line in &lines {
        crate::log_info!(crate::infra::static_logs::messages::LABEL_PRECHECK, line);
    }
}
...
#[cfg(test)]
mod tests {
    use super::{assess_stream_color_info, parse_stream_bit_depth};

    #[test]
    fn parse_stream_bit_depth_prefers_explicit_sample_fields() {
        ...
        assert_eq!(
            parse_stream_bit_depth(&stream, Some("yuv420p")),
            (Some(12), false)
        );
    }

    #[test]
    fn parse_stream_bit_depth_falls_back_to_bits_per_sample_then_pix_fmt() {
        let stream = serde_json::json!({
            "bits_per_sample": "10",
            "pix_fmt": "yuv420p"
        });
        ...
        assert_eq!(
            parse_stream_bit_depth(&inferred, Some("yuv420p10le")),
            (Some(10), true)
        );
    }

    #[test]
    fn assess_stream_color_info_does_not_treat_bt2020_alone_as_hdr() {
        let stream = serde_json::json!({
        ...
        let assessment = assess_stream_color_info(
            &stream,
            Some("bt2020nc"),
            None,
            Some("bt2020"),
    ...
    #[test]
    fn assess_stream_color_info_detects_hdr10_plus_from_side_data() {
        let stream = serde_json::json!({
            "pix_fmt": "yuv420p10le",
            "side_data_list": [{
