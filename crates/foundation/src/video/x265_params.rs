...
fn ram_aware_profile() -> X265MemoryProfile {
    ...
    if let Some((available_mb, total_mb)) =
        crate::media_conversion_gate::delivery_system_memory_mb_or_zero()
    {
...
#[must_use]
pub fn contains_param_key(params: &str, key: &str) -> bool {
    ...
    params
        ...
        .any(|candidate| param_key(candidate) == trimmed_key)
}

#[must_use]
pub fn option_contains_param_key(params: Option<&str>, key: &str) -> bool {
    params.is_some_and(|candidate| contains_param_key(candidate, key))
}

#[must_use]
pub fn has_hdr10plus_metadata(params: Option<&str>) -> bool {
    option_contains_param_key(params, "dhdr10-info")
}

pub fn push_param_if_missing(params: &mut String, param: &str) {
    let trimmed = param.trim_matches(':');
    if trimmed.is_empty() || contains_param_key(params, param_key(trimmed)) {
        return;
    }

    push_param(params, trimmed);
}
...
fn capped_pool_threads(max_threads: usize, profile: X265MemoryProfile) -> usize {
    ...
    let max_threads = crate::performance_schedule::x265_pool_thread_cap(max_threads.max(1), tier);
    match profile {
...
mod tests {
    ...
    fn moderate_profile_injects_moderate_controls() {
        let _guard = crate::common_utils::EnvGuard::set(constants::ENV_MFB_PERF_TIER, "relaxed");
    ...
    #[test]
    fn push_param_if_missing_is_idempotent_by_key() {
        let mut params = "hdr10=1:dhdr10-info=/tmp/hdr10plus.json".to_string();
        push_param_if_missing(&mut params, "hdr10=1");
        push_param_if_missing(&mut params, "dhdr10-info=/tmp/hdr10plus.json");
        push_param_if_missing(&mut params, "repeat-headers=1");

        assert_eq!(
            params,
            "hdr10=1:dhdr10-info=/tmp/hdr10plus.json:repeat-headers=1"
        );
    }

    #[test]
    fn has_hdr10plus_metadata_checks_key_not_substring() {
        assert!(has_hdr10plus_metadata(Some(
            "hdr10=1:dhdr10-info=/tmp/hdr10plus.json"
        )));
        assert!(!has_hdr10plus_metadata(Some("user-data=dhdr10-info")));
        assert!(!has_hdr10plus_metadata(None));
    }
