...
pub fn ffmpeg_timeout() -> Duration {
    let default_secs = crate::constants::FFMPEG_TIMEOUT_SECS;
    match std::env::var(crate::constants::ENV_MFB_FFMPEG_TIMEOUT_SECS) {
        Ok(raw) => {
            ...
            crate::log_warn!(
                ...
                &format!(
                    "ffmpeg timeout env {}={raw:?} invalid; using {}s",
...
impl FfmpegProcess {
    ...
    pub fn spawn(cmd: &mut Command) -> Result<Self> {
        let command_str = crate::common_utils::format_command_for_audit(cmd);
        info!(
            command = %command_str,
    ...
    pub fn wait_with_output_timeout(
        ...
        if let Some(h) = stdout_drain {
            match h.join() {
                Ok(Some(err)) => {
                    crate::media_conversion_gate::delivery_ffmpeg_io_audit(
                        "ffmpeg_stdout_drain_failed",
                        format!("failed to drain FFmpeg bitstream buffers: {err}"),
                    );
                    return Err(anyhow::anyhow!("Failed to drain FFmpeg stdout: {err}"));
                }
                Ok(None) => {}
                Err(_) => {
                    crate::media_conversion_gate::delivery_ffmpeg_io_audit(
                        "ffmpeg_stdout_drain_panicked",
                        crate::infra::static_logs::messages::MSG_FFMPEG_READER_PANIC,
                    );
                    return Err(anyhow::anyhow!("FFmpeg stdout drain thread panicked"));
                }
            }
        }
        ...
        let stderr = match self.stderr_thread.take() {
            Some(thread) => thread.join().map_err(|_| {
                crate::media_conversion_gate::delivery_ffmpeg_io_audit(
                    "ffmpeg_stderr_reader_panicked",
                    crate::infra::static_logs::messages::MSG_FFMPEG_STDERR_PANIC,
                );
                anyhow::anyhow!("FFmpeg stderr reader thread panicked")
            })?,
            None => String::new(),
        };
        ...
        let status = match status_result {
            Ok(status) => status,
            Err(err) => {
                let stderr_tail = crate::io_utils::tail_error_lines(&stderr, 5);
...
impl FfmpegProgressParser {
    ...
    pub fn parse_line(&mut self, line: &str) -> Option<f64> {
        if let Some(frame_str) = line.strip_prefix("frame=") {
            let token = frame_str.split_whitespace().next()?;
            match token.parse::<u64>() {
                Ok(frame) => self.current_frame = frame,
                Err(err) => crate::media_conversion_gate::delivery_progress_batch_audit(
                    "ffmpeg_progress_frame_parse_failed",
                    format!("failed to parse ffmpeg progress frame token {token:?}: {err}"),
                ),
            }
        }

        if let Some(fps_str) = line.strip_prefix("fps=") {
            let token = fps_str.split_whitespace().next()?;
            match token.parse::<f64>() {
                Ok(fps) => self.current_fps = fps,
                Err(err) => crate::media_conversion_gate::delivery_progress_batch_audit(
                    "ffmpeg_progress_fps_parse_failed",
                    format!("failed to parse ffmpeg progress fps token {token:?}: {err}"),
                ),
            }
        }
        ...
        if let Some(speed_str) = line.strip_prefix("speed=") {
            ...
            match speed_str.parse::<f64>() {
                Ok(speed) => self.current_speed = speed,
                Err(err) => crate::media_conversion_gate::delivery_progress_batch_audit(
                    "ffmpeg_progress_speed_parse_failed",
                    format!("failed to parse ffmpeg progress speed token {speed_str:?}: {err}"),
                ),
            }
        }
...
pub fn format_ffmpeg_error(stderr: &str) -> String {
    ...
    crate::media_conversion_gate::probe_ffmpeg_stderr_tail_line_or_unknown(stderr)
}
...
impl std::fmt::Display for FfmpegError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let hint = crate::media_conversion_gate::ui_icon_pick(
            crate::modern_ui::symbols::INFO,
            crate::modern_ui::symbols::plain::INFO,
        );
        writeln!(f, "{err} FFMPEG ERROR")?;
        ...
        if let Some(ref suggestion) = self.suggestion {
            writeln!(f, "   {hint} Suggestion: {suggestion}")?;
        }
        Ok(())
    }
}
...
pub fn run_ffmpeg_with_error_report(args: &[&str]) -> Result<std::process::Output> {
    ...
    let command_str =
        crate::common_utils::format_command_string(crate::constants::TOOL_FFMPEG, args);
