...
pub fn analyze_directory(dir: &Path, config: &AnalysisConfig) -> Result<AnalysisResult, String> {
    if !dir.is_dir() {
        return Err(crate::infra::static_logs::messages::MSG_PROBE_NOT_A_FILE
...
fn extract_best_date(item: &ExiftoolOutput, config: &AnalysisConfig) -> FileDateInfo {
    let filename = crate::media_conversion_gate::delivery_exiftool_field_or_empty(
        item.file_name.clone(),
        crate::infra::static_logs::messages::MSG_DATE_ANOMALY_FILENAME,
    );
    let path = crate::media_conversion_gate::delivery_exiftool_field_or_empty(
        item.source_file.clone(),
        crate::infra::static_logs::messages::MSG_DATE_ANOMALY_SOURCE,
    );
...
fn parse_date(date_str: &str, config: &AnalysisConfig) -> Option<NaiveDateTime> {
    ...
    let clean = crate::media_conversion_gate::str_first_segment_or_whole(
        date_str,
        '+',
        "date_analysis timezone strip",
    );
    ...
    for fmt in formats {
        match NaiveDateTime::parse_from_str(&clean, fmt) {
            Ok(dt) => {
                let year = dt.year();
...
pub fn print_analysis(result: &AnalysisResult) {
    ...
    if !result.by_year.is_empty() {
        ui_stderr::section(
            symbols::CALENDAR_GRID,
            symbols::plain::CALENDAR_GRID,
        ...
        for (year, count) in years {
            ...
            let bar_len = crate::numeric_cast::f64_to_usize_sat(
                (crate::numeric_cast::usize_to_f64(pct) / 3.0).round(),
            ) + 1;
            let bar: String = std::iter::repeat_n(bar_fill, bar_len).collect();
...
mod tests {
    ...
    fn test_parse_date() {
        ...
        assert!(parse_date("1800:01:01 00:00:00", &config).is_none());
    }
