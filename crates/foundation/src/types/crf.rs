    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidCacheKey {
                ...
                write!(f, "Invalid {encoder} CRF cache key {key}: {source}")
            }
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::InvalidCacheKey { source, .. } => Some(source.as_ref()),
            Self::OutOfRange { .. } | Self::InvalidFloat { .. } => None,
        }
    }
}
...
impl<E: EncoderBounds> Crf<E> {
    ...
    pub fn from_cache_key(key: u32) -> Result<Self, Error> {
        ...
        Self::new(value).map_err(|source| Error::InvalidCacheKey {
            key,
            encoder: E::NAME,
