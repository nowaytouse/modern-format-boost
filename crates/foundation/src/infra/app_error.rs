...
fn append_user_detail(msg: &mut String, rendered: impl fmt::Display, context: &'static str) {
    write!(msg, "{rendered}")
        .unwrap_or_else(|_| unreachable!("FORENSIC: String write failed in {context}"));
}

#[inline]
fn user_err_msg(detail: impl fmt::Display) -> String {
    crate::media_conversion_gate::ui_user_facing_error(detail)
}

#[inline]
fn user_warn_msg(detail: impl fmt::Display) -> String {
    crate::media_conversion_gate::ui_user_facing_warning(detail)
}

#[inline]
fn user_msg_skip() -> &'static str {
    crate::modern_ui::symbols::pick("⏭️", "[SKIP]")
}

#[inline]
fn user_msg_hint() -> &'static str {
...
impl AppError {
    ...
    fn io_user_message(&self) -> Option<String> {
        match self {
            Self::FileNotFound { path, operation } => {
                let mut msg = user_err_msg(format!("File not found: {}", path.display()));
                if let Some(op) = operation {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Operation: {op}"),
                        "FileNotFound::user_message",
                    );
                }
                Some(msg)
            }
            Self::DirectoryNotFound { path, operation } => {
                let mut msg = user_err_msg(format!("Directory not found: {}", path.display()));
                if let Some(op) = operation {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Operation: {op}"),
                        "DirectoryNotFound::user_message",
                    );
                }
                Some(msg)
            }
            Self::FileReadError {
                ...
                let mut msg =
                    user_err_msg(format!("Failed to read file {}: {source}", path.display()));
                if let Some(op) = operation {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Operation: {op}"),
                        "FileReadError::user_message",
                    );
                }
                Some(msg)
            }
            Self::FileWriteError {
                ...
                let mut msg =
                    user_err_msg(format!("Failed to write file {}: {source}", path.display()));
                if let Some(op) = operation {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Operation: {op}"),
                        "FileWriteError::user_message",
                    );
                }
                Some(msg)
            }
            Self::Io(e) => Some(user_err_msg(format!("IO error: {e}"))),
            _ => None,
        }
    }
        ...
        match self {
            Self::FfmpegError {
                ...
                let code_str = crate::media_conversion_gate::ui_exit_code_suffix_or_empty(
                    *exit_code,
                    "app_error FFmpeg user message",
                );
                let mut msg = user_err_msg(format!("FFmpeg failed{code_str}: {message}"));
                if let Some(path) = file_path {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   File: {}", path.display()),
                        "FfmpegError::user_message (path)",
                    );
                }
                ...
                if !stderr.is_empty() {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Error output: {stderr}"),
                        "FfmpegError::user_message (stderr)",
                    );
                }
                Some(msg)
            }
            Self::FfprobeError {
                ...
                let mut msg = user_err_msg(format!("FFprobe failed: {message}"));
                if let Some(path) = file_path {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   File: {}", path.display()),
                        "FfprobeError::user_message (path)",
                    );
                }
                if let Some(cmd) = command {
                    append_user_detail(
                ...
                if !stderr.is_empty() {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Error output: {stderr}"),
                        "FfprobeError::user_message (stderr)",
                    );
                }
                Some(msg)
            }
    ...
    fn validation_user_message(&self) -> Option<String> {
        match self {
            Self::InvalidCrf(e) => Some(user_err_msg(format!("Invalid CRF value: {e}"))),
            Self::InvalidSsim(e) => Some(user_err_msg(format!("Invalid SSIM value: {e}"))),
            Self::IterationLimitExceeded(e) => {
                Some(user_warn_msg(format!("Iteration limit exceeded: {e}")))
            }
            Self::CompressionFailed {
                input_size,
                output_size,
                ...
                if let Some(path) = file_path {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   File: {}", path.display()),
                        "CompressionFailed::user_message",
                    );
                }
                Some(msg)
            }
            Self::QualityValidationFailed {
                ...
                if let Some(path) = file_path {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   File: {}", path.display()),
                        "QualityValidationFailed::user_message",
                    );
                }
                Some(msg)
            }
        ...
        match self {
            Self::ToolNotFound {
                ...
                let mut msg = format!(
                    ...
                    user_err_msg(format!("Tool not found: {tool_name}")),
                    user_msg_hint()
                );
                if let Some(op) = operation {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Needed for: {op}"),
                        "ToolNotFound::user_message (state=op_present)",
                    );
                }
                msg
            }
            Self::OutputExists { path, operation } => {
                let mut msg = format!("{} Output file exists: {}", user_msg_skip(), path.display());
                if let Some(op) = operation {
                    append_user_detail(
                        &mut msg,
                        format_args!("\n   Operation: {op}"),
                        "OutputExists::user_message",
                    );
                }
                msg
            }
            Self::Other(e) => user_err_msg(format!("Error: {e}")),
            _ => user_err_msg(format!("Unknown error: {self}")),
        }
    }
