...
enum Commands {
    ...
    /// Run conversion: format-based (JPEG→JXL, PNG→JXL, Animated→HEVC MP4); default explore+match_quality+compress
    ///
    /// 🔥 动态图片/视频转换默认使用智能质量匹配：二分搜索+SSIM 验证+压缩保证
    #[command(name = "run")]
    Run {
...
fn auto_convert_directory(
    ...
    // img_hevc 只负责处理图片。视频文件的处理、未支持文件的复制以及最终完整性校验
