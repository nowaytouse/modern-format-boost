    ...
    let (output_size, final_crf, attempts) = match strategy.target {
        ...
        TargetVideoFormat::Av1Mp4 => {
            if strategy.lossless {
                ...
                // 简化：仅推荐组合有效，统一走 PreciseQualityWithCompress
...
fn execute_ffv1_conversion(
    ...
    let mut args = vec![
        "-y".to_string(),
        "-threads".to_string(),
        max_threads.to_string(), // 限制 ffmpeg 线程数
        "-i".to_string(),
        ...
        "1".to_string(),
        "-slices".to_string(),
        max_threads.to_string(), // 使用与线程数相同的 slices
        "-slicecrc".to_string(),
        "1".to_string(),
    ];
...
fn execute_av1_conversion(
    ...
    let mut args = vec![
        ...
        crf.to_string(),
        "-preset".to_string(),
        "6".to_string(), // 0-13, 6 是平衡点
        "-svtav1-params".to_string(),
        svt_params, // 限制 SVT-AV1 线程数
    ];
...
fn execute_av1_lossless(
    ...
    let mut args = vec![
        ...
        "0".to_string(),
        "-preset".to_string(),
        "4".to_string(), // 无损模式用更慢的 preset 保证质量
        "-svtav1-params".to_string(),
        svt_params, // 数学无损 + 限制线程数
    ];
