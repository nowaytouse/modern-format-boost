    ...
    match result {
        Ok(output_cmd) if output_cmd.status.success() => {
            ...
            let message = if reduction >= 0.0 {
                format!(
                    "AV1 conversion successful: size reduced \x1b[1;32m{reduction_pct:.1}%\x1b[0m"
                )
            } else {
...
pub fn convert_to_av1_mp4_matched(
    ...
    if size_guard_active && explore_result.output_size > max_allowed_size {
        ...
        if options.allow_size_tolerance {
            eprintln!(
                "   ⏭️  Skipping: AV1 output larger than input by {size_increase_pct:.1}% (tolerance: 1.0%)"
            );
        } else {
            eprintln!(
                "   ⏭️  Skipping: AV1 output larger than input by {size_increase_pct:.1}% (strict mode: no tolerance)"
            );
        }
        eprintln!(
        ...
        return Ok(ConversionResult {
            ...
            size_reduction: None,
            message: format!(
                "Skipped: AV1 output larger than input by {size_increase_pct:.1}% ({width}x{height}, tolerance exceeded)"
            ),
            skipped: true,
