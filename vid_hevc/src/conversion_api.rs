...
pub fn determine_strategy_with_apple_compat(
    ...
    // GIF-like video recovery: Telegram / social exports often store a looping
    // GIF sticker as MP4. In Apple compatibility mode we convert such assets
    // back into a real GIF instead of keeping the video container.
    ...
    if gif_like_video && apple_compat && !force {
    ...
    if sticker_meta.is_rhythmic_sticker() && !force {
        return ConversionStrategy {
