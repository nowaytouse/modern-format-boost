...
- **WebP Animated File Loss**: Fixed a critical bug where animated WebP files were silently dropped from the output directory.
  ...
  - **Fix**: Expanded `identify_by_content()` to read **1024 bytes** and implemented a **robust chunk scanner**. It now correctly parses the `VP8X` extended header flag even if non-standard chunks precede it. It returns `WebpAnimated` natively, allowing `vid` to safely collect it without needing `WebpStatic` in the filter.
- **APNG & Modern Format Extension Gap**: Fixed a severe logic gap where animated images masquerading with standard extensions (like an APNG named `.png`) were skipped by `img` (because they are animated) but completely ignored by `vid` (because `.png` was not in `supported_video_extensions()`).
  - **Fix**: Expanded `identify_by_content()` to **robustly scan the first 1024 bytes for the `acTL` chunk**, accurately distinguishing `Apng` from `Png` even when interleaved with metadata chunks (like iCCP, gAMA, sRGB).
  - **Fix**: Safely added `"png"`, `"apng"`, `"jxl"`, and `"heif"` to the `vid` tool's `SUPPORTED_VIDEO_EXTENSIONS` list. Because the robust shallow probe now accurately distinguishes static vs animated variants natively, `vid` can safely scan `.png` folders at lightning speed without triggering expensive `ffprobe` operations on static files.
