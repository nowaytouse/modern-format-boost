...
- **Bash `tee` Output Crash on SIGINT**: Modified the Apple wrapper and script pipelines to use a `(trap '' INT; tee)` wrapper so that `tee` doesn't immediately exit when users issue a Ctrl+C, thereby preventing silent `SIGPIPE` crashes in the underlying Rust binaries.
