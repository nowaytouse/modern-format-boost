...
- **Bash `tee` Output Crash on SIGINT**: Modified the Apple wrapper and script pipelines to use a `(trap '' INT; tee)` wrapper so that `tee` doesn't immediately exit when users issue a Ctrl+C, thereby preventing silent `SIGPIPE` crashes in the underlying Rust binaries. Additionally, fixed a bug where the background Bash spinner in the terminal title bar leaked out and continued ticking after a user terminated the Rust binaries; Bash now explicitly tests pipelines for `130` and leverages an `EXIT` trap to gracefully shut down the spinner.
