...
def parse_logs(log_paths, output_report):
                ...
                with open(log_file, encoding="utf-8", errors="ignore") as f:
                    for line in f:
                        ...
                        if match:
                            ...
                            if src_ext in modern_exts and (
                                tgt_ext.strip(".") in target_formats
                                or "GIF" in message
                                or "MOV" in message
                            ):
                                results.append(
                        ...
                        if match:
                            ...
                            src_ext = Path(source).suffix.lower()
                            if src_ext in modern_exts and (
                                target_fmt in target_formats
                                or "GIF" in target_fmt
                                or "MOV" in target_fmt
                            ):
                                results.append(
                                    {
                                        "log": log_file.name,
    ...
    with open(output_report, "w", encoding="utf-8") as f:
        ...
        if not unique_results:
            f.write(
                "No 'Modern → Legacy' conversions (WebP/AVIF/JXL to GIF/MOV) found in logs.\n"
            )
        else:
            f.write(
                f"Found {len(unique_results)} modern-to-legacy conversion events:\n\n"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze MFB logs for modern format conversions to GIF/MOV."
    )
    parser.add_argument(
        "logs", nargs="+", help="One or more log files or directories to scan."
    )
    ...
    output_report = (
        args.output if args.output else f"logs/conversion_summary_{timestamp}.txt"
    )
