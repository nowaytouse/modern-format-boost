...
class TestCollectOptimized(unittest.TestCase):
    ...
    def test_get_finder_comment_mdls_success(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="kMDItemFinderComment = \"[Optimized by Modern Format Boost]\"")
    ...
    @patch('os.utime')
    @patch('os.stat')
    def test_full_collection_flow(self, mock_stat, mock_utime, mock_move, mock_mkdir, mock_exists, mock_isdir, mock_walk, mock_comment):
        # Setup mocks
        mock_isdir.return_value = True
    ...
    @patch('os.path.exists')
    @patch('shutil.move')
    def test_dry_run_no_moves(self, mock_move, mock_exists, mock_isdir, mock_walk, mock_comment):
        mock_isdir.return_value = True
