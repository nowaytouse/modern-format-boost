...
def create_live_photo(video_path: str, output_dir: str = None, 
                      photo_format: str = "jpg", hq: bool = False,
    ...
    check_dependencies(needs_heif=(photo_format == "heic"), needs_makelive=inject_metadata)
    ...
    video_duration = min(duration, 3.0) # Live Photo videos are usually limited to 3 seconds
    ...
    if photo_format == "heic":
        ...
        subprocess.run([
            ...
            "-pix_fmt", "rgb24", str(temp_png)
        ], check=True, capture_output=True)
        
        subprocess.run([
            "heif-enc", "--lossless" if hq else "-q", "85",
            "-o", str(img_path), str(temp_png)
        ], check=True, capture_output=True)
    ...
    print("\n" + "="*50)
    print("✨ Live Photo creation process complete!")
    print("="*50)
...
def main():
    parser = argparse.ArgumentParser(description="Convert video to Apple Live Photo format")
    parser.add_argument("video", help="Input video file path")
    parser.add_argument("-o", "--output", help="Output directory (optional)")
    parser.add_argument("-f", "--format", choices=["jpg", "heic"], default="jpg", help="Image format (default: jpg)")
    parser.add_argument("--hq", action="store_true", help="Enable high-quality encoding mode")
    parser.add_argument("--no-meta", action="store_true", help="Disable makelive metadata injection")
