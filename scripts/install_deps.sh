...
if [[ "$OS_TYPE" == "darwin" ]]; then
    ...
    brew install ffmpeg jpeg-xl exiftool imagemagick webp libheif coreutils node shellcheck shfmt
elif [[ "$OS_TYPE" == "linux" ]]; then
    echo -e "${YELLOW}🐧 Detected Linux${NC}"
    if command_exists apt-get; then
        echo "Installing system dependencies via apt..."
        sudo apt-get update
        sudo apt-get install -y ffmpeg libimage-exiftool-perl imagemagick webp libheif-dev coreutils nodejs npm shellcheck shfmt curl git build-essential
    else
