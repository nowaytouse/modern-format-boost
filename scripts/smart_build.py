...
def clean_with_kondo():
    ...
    subprocess.run(["kondo", "-n", "-I", "/Volumes", "-I", os.path.expanduser("~/Library"), "."])
    print()
...
def build_project(project_dir, binary_name, retry_count, args):
    ...
    cmd = ["cargo", "build", "--release", "--manifest-path", f"{project_dir}/Cargo.toml"]
    ...
    if args.verify_timestamps:
        ...
        if not verify_binary_timestamp(binary_path, compile_start_time):
            if retry_count < 2:
                print(f"{YELLOW}Retry {retry_count + 1}/2: Rebuilding with clean...{NC}")
                ...
                print(f"{RED}FAILURE: Timestamp verification failed after 2 retries{NC}")
                return False
    return True
...
def main():
    parser = argparse.ArgumentParser(description="Smart Build System v0.11.2 (Python Edition)")
    parser.add_argument("--force", "-f", action="store_true", help="Force rebuild all selected projects")
    parser.add_argument("--clean", "-c", action="store_true", help="Clean build artifacts before compiling")
    parser.add_argument("--verbose", "-v", action="store_true", help="Show detailed output")
    parser.add_argument("--no-clean-old", dest="clean_old", action="store_false", help="Don't clean old binary files")
    ...
    parser.add_argument("--kondo", action="store_true", help="Perform deep project cleanup using kondo")
    parser.add_argument("--no-verify-timestamps", dest="verify_timestamps", action="store_false", help="Disable timestamp verification after build")
    ...
    if args.all:
        ...
        if args.hevc or args.av1:
            if "crates/img" not in projects_to_build: projects_to_build.append("crates/img")
            if "crates/vid" not in projects_to_build: projects_to_build.append("crates/vid")
